import { app as aE, screen as Nn, BrowserWindow as f_, globalShortcut as SA, ipcMain as p, Notification as Tn, Menu as en, nativeImage as nI, Tray as II } from "electron";
import { fileURLToPath as NI } from "node:url";
import r_, { randomUUID as TI } from "crypto";
import x from "node:path";
import GE from "events";
import d_ from "process";
import eI from "net";
import tn from "tls";
import dA from "timers";
import an from "stream";
import On from "buffer";
import tI from "string_decoder";
import iI from "zlib";
import Cn from "util";
import aI from "url";
import P_ from "node:fs";
...
(function(E) {
  ...
  _.escapeId = function(T, a) {
    if (Array.isArray(T)) {
      for (var u = "", S = 0; S < T.length; S++)
        u += (S === 0 ? "" : ", ") + _.escapeId(T[S], a);
      return u;
    } else return a ? "`" + String(T).replace(A, "``") + "`" : "`" + String(T).replace(A, "``").replace(R, "`.`") + "`";
  }, _.escape = function(T, a, u) {
    if (T == null)
      return "NULL";
    switch (typeof T) {
      case "boolean":
      ...
      case "number":
        return T + "";
      case "object":
        return Object.prototype.toString.call(T) === "[object Date]" ? _.dateToString(T, u || "local") : Array.isArray(T) ? _.arrayToList(T, u) : Buffer.isBuffer(T) ? _.bufferToString(T) : typeof T.toSqlString == "function" ? String(T.toSqlString()) : a ? N(T.toString()) : _.objectToValues(T, u);
      default:
        return N(T);
    }
  }, _.arrayToList = function(T, a) {
    for (var u = "", S = 0; S < T.length; S++) {
      var O = T[S];
      Array.isArray(O) ? u += (S === 0 ? "" : ", ") + "(" + _.arrayToList(O, a) + ")" : u += (S === 0 ? "" : ", ") + _.escape(O, !0, a);
    }
    return u;
  }, _.format = function(T, a, u, S) {
    if (a == null)
      return T;
    Array.isArray(a) || (a = [a]);
    for (var O = 0, D = /\?+/g, c = "", h = 0, M; h < a.length && (M = D.exec(T)); ) {
      var L = M[0].length;
      if (!(L > 2)) {
        var B = L === 2 ? _.escapeId(a[h]) : _.escape(a[h], u, S);
        c += T.slice(O, M.index) + B, O = D.lastIndex, h++;
      }
    }
    return O === 0 ? T : O < T.length ? c + T.slice(O) : c;
  }, _.dateToString = function(T, a) {
    var u = new Date(T);
    if (isNaN(u.getTime()))
      return "NULL";
    var S, O, D, c, h, M, L;
    if (a === "local")
      S = u.getFullYear(), O = u.getMonth() + 1, D = u.getDate(), c = u.getHours(), h = u.getMinutes(), M = u.getSeconds(), L = u.getMilliseconds();
    else {
      var B = i(a);
      B !== !1 && B !== 0 && u.setTime(u.getTime() + B * 6e4), S = u.getUTCFullYear(), O = u.getUTCMonth() + 1, D = u.getUTCDate(), c = u.getUTCHours(), h = u.getUTCMinutes(), M = u.getUTCSeconds(), L = u.getUTCMilliseconds();
    }
    var s = t(S, 4) + "-" + t(O, 2) + "-" + t(D, 2) + " " + t(c, 2) + ":" + t(h, 2) + ":" + t(M, 2) + "." + t(L, 3);
    return N(s);
  }, _.bufferToString = function(T) {
    return "X" + N(T.toString("hex"));
  }, _.objectToValues = function(T, a) {
    var u = "";
    for (var S in T) {
      var O = T[S];
      typeof O != "function" && (u += (u.length === 0 ? "" : ", ") + _.escapeId(S) + " = " + _.escape(O, !0, a));
    }
    return u;
  }, _.raw = function(T) {
    if (typeof T != "string")
  ...
  function N(e) {
    for (var T = n.lastIndex = 0, a = "", u; u = n.exec(e); )
      a += e.slice(T, u.index) + I[u[0]], T = n.lastIndex;
    return T === 0 ? "'" + e + "'" : T < e.length ? "'" + a + e.slice(T) + "'" : "'" + a + "'";
  }
  function t(e, T) {
...
const DI = (E) => {
  ...
  let A = 0, R = 0, n = 0, I = [];
  const { onEviction: N } = E, t = /* @__PURE__ */ new Map(), i = new Array(_).fill(void 0), e = new Array(_).fill(void 0), T = new Array(_).fill(0), a = new Array(_).fill(0), u = (O, D) => {
    if (O === n)
      return;
    const c = T[O], h = a[O];
    O === R ? R = c : (D === "get" || h !== 0) && (T[h] = c), c !== 0 && (a[c] = h), T[n] = O, a[O] = n, T[O] = 0, n = O;
  }, S = () => {
    const O = R, D = i[O];
    return N == null || N(D, e[O]), t.delete(D), i[O] = void 0, e[O] = void 0, R = T[O], R !== 0 && (a[R] = 0), A--, A === 0 && (R = n = 0), I.push(O), O;
  };
  return {
    ...
    set(O, D) {
      if (O === void 0)
        return;
      let c = t.get(O);
      c === void 0 ? (c = A === _ ? S() : I.length > 0 ? I.pop() : A, t.set(O, c), i[c] = O, A++) : N == null || N(O, e[c]), e[c] = D, A === 1 ? R = n = c : u(c, "set");
    },
    /** Retrieves the value for a given key and moves the key to the most recent position. */
    get(O) {
      const D = t.get(O);
      if (D !== void 0)
        return D !== n && u(D, "get"), e[D];
    },
    /** Retrieves the value for a given key without changing its position. */
    peek: (O) => {
      const D = t.get(O);
      return D !== void 0 ? e[D] : void 0;
    },
    ...
    *entries() {
    ...
    delete(O) {
      ...
      if (D === void 0)
        return !1;
      N == null || N(O, e[D]), t.delete(O), I.push(D), i[D] = void 0, e[D] = void 0;
      const c = a[D], h = T[D];
      return c !== 0 && (T[c] = h), h !== 0 && (a[h] = c), D === R && (R = h), D === n && (n = c), A--, !0;
    },
    ...
    clear() {
      ...
      t.clear(), i.fill(void 0), e.fill(void 0), I = [], A = 0, R = n = 0;
    },
    ...
    resize: (O) => {
      ...
      if (O !== _) {
        if (O < _) {
          let D = n;
          const c = Math.min(A, O), h = A - c, M = new Array(O), L = new Array(O), B = new Array(O), s = new Array(O);
          for (let l = 1; l <= h; l++)
            N == null || N(i[l], e[l]);
          for (let l = c - 1; l >= 0; l--)
            M[l] = i[D], L[l] = e[D], B[l] = l + 1, s[l] = l - 1, t.set(M[l], l), D = a[D];
          R = 0, n = c - 1, A = c, i.length = O, e.length = O, T.length = O, a.length = O;
          for (let l = 0; l < c; l++)
            i[l] = M[l], e[l] = L[l], T[l] = B[l], a[l] = s[l];
          I = [];
          for (let l = c; l < O; l++)
            I.push(l);
        } else {
          const D = O - _;
          i.push(...new Array(D).fill(void 0)), e.push(...new Array(D).fill(void 0)), T.push(...new Array(D).fill(0)), a.push(...new Array(D).fill(0));
        }
        _ = O;
      }
    },
...
function Sn(E, _, A, R) {
  const n = [
    ...
    A.nestTables,
    !!A.rowsAsArray,
    !!(A.supportBigNumbers || R.supportBigNumbers),
    !!(A.bigNumberStrings || R.bigNumberStrings),
    typeof A.typeCast == "boolean" ? A.typeCast : typeof A.typeCast,
...
function SI(E, _, A, R, n) {
  const I = Sn(E, _, A, R);
  let N = l_.get(I);
  return N || (N = n(_, A, R), l_.set(I, N), N);
}
function cI(E) {
  l_.resize(E);
}
...
function v(E, A) {
  var A = A || {};
  this._capacity = A.capacity, this._head = 0, this._tail = 0, Array.isArray(E) ? this._fromArray(E) : (this._capacityMask = 3, this._list = new Array(4));
}
v.prototype.peekAt = function(_) {
  var A = _;
  if (A === (A | 0)) {
    var R = this.size();
    if (!(A >= R || A < -R))
      return A < 0 && (A += R), A = this._head + A & this._capacityMask, this._list[A];
  }
};
...
v.prototype.peekFront = function() {
  return this.peek();
};
v.prototype.peekBack = function() {
...
v.prototype.unshift = function(_) {
  if (arguments.length === 0) return this.size();
  var A = this._list.length;
  return this._head = this._head - 1 + A & this._capacityMask, this._list[this._head] = _, this._tail === this._head && this._growArray(), this._capacity && this.size() > this._capacity && this.pop(), this._head < this._tail ? this._tail - this._head : this._capacityMask + 1 - (this._head - this._tail);
};
v.prototype.shift = function() {
  var _ = this._head;
  if (_ !== this._tail) {
    var A = this._list[_];
    return this._list[_] = void 0, this._head = _ + 1 & this._capacityMask, _ < 2 && this._tail > 1e4 && this._tail <= this._list.length >>> 2 && this._shrinkArray(), A;
  }
};
v.prototype.push = function(_) {
  if (arguments.length === 0) return this.size();
  var A = this._tail;
  return this._list[A] = _, this._tail = A + 1 & this._capacityMask, this._tail === this._head && this._growArray(), this._capacity && this.size() > this._capacity && this.shift(), this._head < this._tail ? this._tail - this._head : this._capacityMask + 1 - (this._head - this._tail);
};
v.prototype.pop = function() {
  var _ = this._tail;
  if (_ !== this._head) {
    var A = this._list.length;
    this._tail = _ - 1 + A & this._capacityMask;
    var R = this._list[this._tail];
    return this._list[this._tail] = void 0, this._head < 2 && _ > 1e4 && _ <= A >>> 2 && this._shrinkArray(), R;
  }
};
v.prototype.removeOne = function(_) {
  var A = _;
  if (A === (A | 0) && this._head !== this._tail) {
    var R = this.size(), n = this._list.length;
    if (!(A >= R || A < -R)) {
      A < 0 && (A += R), A = this._head + A & this._capacityMask;
      var I = this._list[A], N;
      if (_ < R / 2) {
        for (N = _; N > 0; N--)
          this._list[A] = this._list[A = A - 1 + n & this._capacityMask];
        this._list[A] = void 0, this._head = this._head + 1 + n & this._capacityMask;
      } else {
        for (N = R - 1 - _; N > 0; N--)
          this._list[A] = this._list[A = A + 1 + n & this._capacityMask];
        this._list[A] = void 0, this._tail = this._tail - 1 + n & this._capacityMask;
      }
      return I;
    }
  }
};
v.prototype.remove = function(_, A) {
  var R = _, n, I = A;
  if (R === (R | 0) && this._head !== this._tail) {
    var N = this.size(), t = this._list.length;
    if (!(R >= N || R < -N || A < 1)) {
      if (R < 0 && (R += N), A === 1 || !A)
      ...
      R + A > N && (A = N - R);
      var i;
      for (n = new Array(A), i = 0; i < A; i++)
        n[i] = this._list[this._head + R + i & this._capacityMask];
      if (R = this._head + R & this._capacityMask, _ + A === N) {
        for (this._tail = this._tail - A + t & this._capacityMask, i = A; i > 0; i--)
          this._list[R = R + 1 + t & this._capacityMask] = void 0;
        return n;
      }
      if (_ === 0) {
        for (this._head = this._head + A + t & this._capacityMask, i = A - 1; i > 0; i--)
          this._list[R = R + 1 + t & this._capacityMask] = void 0;
        return n;
      }
      if (R < N / 2) {
        for (this._head = this._head + _ + A + t & this._capacityMask, i = _; i > 0; i--)
          this.unshift(this._list[R = R - 1 + t & this._capacityMask]);
        for (R = this._head - 1 + t & this._capacityMask; I > 0; )
          this._list[R = R - 1 + t & this._capacityMask] = void 0, I--;
        _ < 0 && (this._tail = R);
      } else {
        for (this._tail = R, R = R + A + t & this._capacityMask, i = N - (A + _); i > 0; i--)
          this.push(this._list[R++]);
        for (R = this._tail; I > 0; )
          this._list[R = R + 1 + t & this._capacityMask] = void 0, I--;
      }
      return this._head < 2 && this._tail > 1e4 && this._tail <= t >>> 2 && this._shrinkArray(), n;
    }
  }
};
v.prototype.splice = function(_, A) {
  var R = _;
  if (R === (R | 0)) {
    var n = this.size();
    if (R < 0 && (R += n), !(R > n))
      if (arguments.length > 2) {
        var I, N, t, i = arguments.length, e = this._list.length, T = 2;
        if (!n || R < n / 2) {
          for (N = new Array(R), I = 0; I < R; I++)
            N[I] = this._list[this._head + I & this._capacityMask];
          for (A === 0 ? (t = [], R > 0 && (this._head = this._head + R + e & this._capacityMask)) : (t = this.remove(R, A), this._head = this._head + R + e & this._capacityMask); i > T; )
          ...
          var a = N.length;
          for (I = 0; I < a; I++)
            N[I] = this._list[this._head + R + A + I & this._capacityMask];
          for (A === 0 ? (t = [], R != n && (this._tail = this._head + R + e & this._capacityMask)) : (t = this.remove(R, A), this._tail = this._tail - a + e & this._capacityMask); T < i; )
...
v.prototype._fromArray = function(_) {
  var A = _.length, R = this._nextPowerOf2(A);
  this._list = new Array(R), this._capacityMask = R - 1, this._tail = A;
  for (var n = 0; n < A; n++) this._list[n] = _[n];
};
v.prototype._copyArray = function(_, A) {
  var R = this._list, n = R.length, I = this.length;
  if (A = A | I, A == I && this._head < this._tail)
    return this._list.slice(this._head, this._tail);
  var N = new Array(A), t = 0, i;
  if (_ || this._head > this._tail) {
    for (i = this._head; i < n; i++) N[t++] = R[i];
    for (i = 0; i < this._tail; i++) N[t++] = R[i];
  } else
    for (i = this._head; i < this._tail; i++) N[t++] = R[i];
  return N;
};
v.prototype._growArray = function() {
  if (this._head != 0) {
...
v.prototype._shrinkArray = function() {
  this._list.length >>>= 1, this._capacityMask >>>= 1;
};
v.prototype._nextPowerOf2 = function(_) {
  var A = Math.log(_) / Math.log(2), R = 1 << A + 1;
  return Math.max(R, 4);
};
var cn = v, FA = {};
(function(E) {
...
(function(E, _) {
  (function(A, R) {
    ...
    typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : OI,
    function(A) {
      ...
      function n(U, C, f) {
        this.low = U | 0, this.high = C | 0, this.unsigned = !!f;
      }
      ...
      function e(U, C) {
        var f, w, Y;
        return C ? (U >>>= 0, (Y = 0 <= U && U < 256) && (w = i[U], w) ? w : (f = a(U, 0, !0), Y && (i[U] = f), f)) : (U |= 0, (Y = -128 <= U && U < 128) && (w = t[U], w) ? w : (f = a(U, U < 0 ? -1 : 0, !1), Y && (t[U] = f), f));
      }
      n.fromInt = e;
      function T(U, C) {
        if (isNaN(U)) return C ? l : s;
        if (C) {
          if (U < 0) return l;
          if (U >= M) return OE;
        } else {
      ...
      function S(U, C, f) {
        ...
        if (typeof C == "number" ? (f = C, C = !1) : C = !!C, U === "NaN" || U === "Infinity" || U === "+Infinity" || U === "-Infinity")
          return C ? l : s;
        if (f = f || 10, f < 2 || 36 < f) throw RangeError("radix");
        var w;
        if ((w = U.indexOf("-")) > 0) throw Error("interior hyphen");
        if (w === 0)
          return S(U.substring(1), C, f).neg();
        for (var Y = T(u(f, 8)), P = s, j = 0; j < U.length; j += 8) {
          var W = Math.min(8, U.length - j), TE = parseInt(U.substring(j, j + W), f);
          if (W < 8) {
            var EE = T(u(f, W));
            P = P.mul(EE).add(T(TE));
          } else
            P = P.mul(Y), P = P.add(T(TE));
        }
        return P.unsigned = C, P;
      }
      n.fromString = S;
      function O(U, C) {
        return typeof U == "number" ? T(U, C) : typeof U == "string" ? S(U, C) : a(
          U.low,
          U.high,
          typeof C == "boolean" ? C : U.unsigned
        );
      }
      n.fromValue = O;
      var D = 65536, c = 1 << 24, h = D * D, M = h * h, L = M / 2, B = e(c), s = e(0);
      n.ZERO = s;
      var l = e(0, !0);
      n.UZERO = l;
      var d = e(1);
      ...
      var Z = e(-1);
      n.NEG_ONE = Z;
      var RE = a(-1, 2147483647, !1);
      n.MAX_VALUE = RE;
      var OE = a(-1, -1, !0);
      n.MAX_UNSIGNED_VALUE = OE;
      ...
      g.toInt = function() {
        ...
        if (this.isNegative())
          if (this.eq(r)) {
            var f = T(C), w = this.div(f), Y = w.mul(f).sub(this);
            return w.toString(C) + Y.toInt().toString(C);
          } else return "-" + this.neg().toString(C);
        for (var P = T(u(C, 6), this.unsigned), j = this, W = ""; ; ) {
          var TE = j.div(P), EE = j.sub(TE.mul(P)).toInt() >>> 0, J = EE.toString(C);
          if (j = TE, j.isZero()) return J + W;
        ...
        for (var C = this.high != 0 ? this.high : this.low, f = 31; f > 0 && !(C & 1 << f); f--) ;
        return this.high != 0 ? f + 33 : f + 1;
      }, g.isSafeInteger = function() {
        var C = this.high >> 21;
        return C ? this.unsigned ? !1 : C === -1 && !(this.low === 0 && this.high === -2097152) : !0;
      }, g.isZero = function() {
        ...
        return I(C) || (C = O(C)), this.unsigned !== C.unsigned && this.high >>> 31 === 1 && C.high >>> 31 === 1 ? !1 : this.high === C.high && this.low === C.low;
      }, g.eq = g.equals, g.notEquals = function(C) {
        return !this.eq(
          /* validates */
          C
        );
      }, g.neq = g.notEquals, g.ne = g.notEquals, g.lessThan = function(C) {
        return this.comp(
          /* validates */
          C
        ) < 0;
      }, g.lt = g.lessThan, g.lessThanOrEqual = function(C) {
        return this.comp(
          /* validates */
          C
        ) <= 0;
      }, g.lte = g.lessThanOrEqual, g.le = g.lessThanOrEqual, g.greaterThan = function(C) {
        return this.comp(
          /* validates */
          C
        ) > 0;
      }, g.gt = g.greaterThan, g.greaterThanOrEqual = function(C) {
        return this.comp(
          /* validates */
          C
        ) >= 0;
      }, g.gte = g.greaterThanOrEqual, g.ge = g.greaterThanOrEqual, g.compare = function(C) {
        if (I(C) || (C = O(C)), this.eq(C)) return 0;
        var f = this.isNegative(), w = C.isNegative();
        return f && !w ? -1 : !f && w ? 1 : this.unsigned ? C.high >>> 0 > this.high >>> 0 || C.high === this.high && C.low >>> 0 > this.low >>> 0 ? -1 : 1 : this.sub(C).isNegative() ? -1 : 1;
      }, g.comp = g.compare, g.negate = function() {
        return !this.unsigned && this.eq(r) ? r : this.not().add(d);
      }, g.neg = g.negate, g.add = function(C) {
        I(C) || (C = O(C));
        var f = this.high >>> 16, w = this.high & 65535, Y = this.low >>> 16, P = this.low & 65535, j = C.high >>> 16, W = C.high & 65535, TE = C.low >>> 16, EE = C.low & 65535, J = 0, eE = 0, k = 0, nE = 0;
        return nE += P + EE, k += nE >>> 16, nE &= 65535, k += Y + TE, eE += k >>> 16, k &= 65535, eE += w + W, J += eE >>> 16, eE &= 65535, J += f + j, J &= 65535, a(k << 16 | nE, J << 16 | eE, this.unsigned);
      }, g.subtract = function(C) {
        return I(C) || (C = O(C)), this.add(C.neg());
      }, g.sub = g.subtract, g.multiply = function(C) {
        ...
        if (I(C) || (C = O(C)), R) {
          ...
          return a(f, R.get_high(), this.unsigned);
        }
        if (C.isZero()) return this.unsigned ? l : s;
        if (this.eq(r)) return C.isOdd() ? r : s;
        if (C.eq(r)) return this.isOdd() ? r : s;
        if (this.isNegative())
        ...
        if (this.lt(B) && C.lt(B))
          return T(
            this.toNumber() * C.toNumber(),
            this.unsigned
          );
        var w = this.high >>> 16, Y = this.high & 65535, P = this.low >>> 16, j = this.low & 65535, W = C.high >>> 16, TE = C.high & 65535, EE = C.low >>> 16, J = C.low & 65535, eE = 0, k = 0, nE = 0, T_ = 0;
        return T_ += j * J, nE += T_ >>> 16, T_ &= 65535, nE += P * J, k += nE >>> 16, nE &= 65535, nE += j * EE, k += nE >>> 16, nE &= 65535, k += Y * J, eE += k >>> 16, k &= 65535, k += P * EE, eE += k >>> 16, k &= 65535, k += j * TE, eE += k >>> 16, k &= 65535, eE += w * J + Y * EE + P * TE + j * W, eE &= 65535, a(nE << 16 | T_, eE << 16 | k, this.unsigned);
      }, g.mul = g.multiply, g.divide = function(C) {
        if (I(C) || (C = O(C)), C.isZero()) throw Error("division by zero");
        if (R) {
          if (!this.unsigned && this.high === -2147483648 && C.low === -1 && C.high === -1)
            return this;
          var f = (this.unsigned ? R.div_u : R.div_s)(
            this.low,
            this.high,
            C.low,
            C.high
        ...
        if (this.isZero()) return this.unsigned ? l : s;
        var w, Y, P;
        if (this.unsigned) {
          if (C.unsigned || (C = C.toUnsigned()), C.gt(this)) return l;
          if (C.gt(this.shru(1)))
            return o;
          P = l;
        } else {
          if (this.eq(r)) {
            ...
            if (C.eq(r)) return d;
            var j = this.shr(1);
            return w = j.div(C).shl(1), w.eq(s) ? C.isNegative() ? d : Z : (Y = this.sub(C.mul(w)), P = w.add(Y.div(C)), P);
          } else if (C.eq(r)) return this.unsigned ? l : s;
          if (this.isNegative())
        ...
        for (Y = this; Y.gte(C); ) {
          w = Math.max(1, Math.floor(Y.toNumber() / C.toNumber()));
          for (var W = Math.ceil(Math.log(w) / Math.LN2), TE = W <= 48 ? 1 : u(2, W - 48), EE = T(w), J = EE.mul(C); J.isNegative() || J.gt(Y); )
            w -= TE, EE = T(w, this.unsigned), J = EE.mul(C);
          EE.isZero() && (EE = d), P = P.add(EE), Y = Y.sub(J);
        }
        return P;
      }, g.div = g.divide, g.modulo = function(C) {
        if (I(C) || (C = O(C)), R) {
          var f = (this.unsigned ? R.rem_u : R.rem_s)(
            this.low,
            this.high,
            C.low,
            C.high
      ...
      }, g.countLeadingZeros = function() {
        return this.high ? Math.clz32(this.high) : Math.clz32(this.low) + 32;
      }, g.clz = g.countLeadingZeros, g.countTrailingZeros = function() {
        return this.low ? N(this.low) : N(this.high) + 32;
      }, g.ctz = g.countTrailingZeros, g.and = function(C) {
        return I(C) || (C = O(C)), a(
          this.low & C.low,
          this.high & C.high,
          this.unsigned
      ...
      }, g.shiftLeft = function(C) {
        return I(C) && (C = C.toInt()), (C &= 63) === 0 ? this : C < 32 ? a(
          this.low << C,
          this.high << C | this.low >>> 32 - C,
          this.unsigned
        ) : a(0, this.low << C - 32, this.unsigned);
      }, g.shl = g.shiftLeft, g.shiftRight = function(C) {
        return I(C) && (C = C.toInt()), (C &= 63) === 0 ? this : C < 32 ? a(
          this.low >>> C | this.high << 32 - C,
          this.high >> C,
          this.unsigned
        ) : a(
          this.high >> C - 32,
      ...
      }, g.shr = g.shiftRight, g.shiftRightUnsigned = function(C) {
        return I(C) && (C = C.toInt()), (C &= 63) === 0 ? this : C < 32 ? a(
          this.low >>> C | this.high << 32 - C,
          this.high >>> C,
          this.unsigned
        ) : C === 32 ? a(this.high, 0, this.unsigned) : a(this.high >>> C - 32, 0, this.unsigned);
      }, g.shru = g.shiftRightUnsigned, g.shr_u = g.shiftRightUnsigned, g.rotateLeft = function(C) {
        var f;
        return I(C) && (C = C.toInt()), (C &= 63) === 0 ? this : C === 32 ? a(this.high, this.low, this.unsigned) : C < 32 ? (f = 32 - C, a(
          this.low << C | this.high >>> f,
      ...
      }, g.rotl = g.rotateLeft, g.rotateRight = function(C) {
        var f;
        return I(C) && (C = C.toInt()), (C &= 63) === 0 ? this : C === 32 ? a(this.high, this.low, this.unsigned) : C < 32 ? (f = 32 - C, a(
          this.high << f | this.low >>> C,
          this.low << f | this.high >>> C,
          this.unsigned
        )) : (C -= 32, f = 32 - C, a(
          this.low << f | this.high >>> C,
          this.high << f | this.low >>> C,
      ...
      }, n.fromBytes = function(C, f, w) {
        return w ? n.fromBytesLE(C, f) : n.fromBytesBE(C, f);
      }, n.fromBytesLE = function(C, f) {
        return new n(
          C[0] | C[1] << 8 | C[2] << 16 | C[3] << 24,
          C[4] | C[5] << 8 | C[6] << 16 | C[7] << 24,
          f
        );
      }, n.fromBytesBE = function(C, f) {
        return new n(
          C[4] << 24 | C[5] << 16 | C[6] << 8 | C[7],
          C[0] << 24 | C[1] << 16 | C[2] << 8 | C[3],
          f
        );
      }, typeof BigInt == "function" && (n.fromBigInt = function(C, f) {
        var w = Number(BigInt.asIntN(32, C)), Y = Number(BigInt.asIntN(32, C >> BigInt(32)));
        return a(w, Y, f);
      }, n.fromValue = function(C, f) {
        return typeof C == "bigint" ? n.fromBigInt(C, f) : O(C, f);
      }, g.toBigInt = function() {
        var C = BigInt(this.low >>> 0), f = BigInt(this.unsigned ? this.high >>> 0 : this.high);
        return f << BigInt(32) | C;
      }), A.default = n;
    }
  );
})(cA, cA.exports);
var MI = cA.exports, zE = {}, QA = { exports: {} }, h_ = On, KE = h_.Buffer, tE = {}, iE;
for (iE in h_)
  h_.hasOwnProperty(iE) && (iE === "SlowBuffer" || iE === "Buffer" || (tE[iE] = h_[iE]));
var jE = tE.Buffer = {};
for (iE in KE)
  KE.hasOwnProperty(iE) && (iE === "allocUnsafe" || iE === "allocUnsafeSlow" || (jE[iE] = KE[iE]));
tE.Buffer.prototype = KE.prototype;
(!jE.from || jE.from === Uint8Array.from) && (jE.from = function(E, _, A) {
  if (typeof E == "number")
...
jE.alloc || (jE.alloc = function(E, _, A) {
  if (typeof E != "number")
    throw new TypeError('The "size" argument must be of type number. Received type ' + typeof E);
  if (E < 0 || E >= 2 * (1 << 30))
    throw new RangeError('The value "' + E + '" is invalid for option "size"');
  var R = KE(E);
  return !_ || _.length === 0 ? R.fill(0) : typeof A == "string" ? R.fill(_, A) : R.fill(_), R;
});
if (!tE.kStringMaxLength)
  try {
    tE.kStringMaxLength = process.binding("buffer").kStringMaxLength;
  } catch {
  }
tE.constants || (tE.constants = {
  MAX_LENGTH: tE.kMaxLength
}, tE.kStringMaxLength && (tE.constants.MAX_STRING_LENGTH = tE.kStringMaxLength));
var wE = tE, PA = {}, Bn = "\uFEFF";
PA.PrependBOM = VA;
function VA(E, _) {
...
VA.prototype.write = function(E) {
  return this.addBOM && (E = Bn + E, this.addBOM = !1), this.encoder.write(E);
};
...
bA.prototype.write = function(E) {
  ...
  return this.pass || !_ || (_[0] === Bn && (_ = _.slice(1), typeof this.options.stripBOM == "function" && this.options.stripBOM()), this.pass = !0), _;
};
bA.prototype.end = function() {
  return this.decoder.end();
};
var LI = typeof Object.hasOwn > "u" ? Function.call.bind(Object.prototype.hasOwnProperty) : Object.hasOwn;
function hI(E, _) {
  for (var A in _)
    LI(_, A) && (E[A] = _[A]);
}
...
function UI() {
  ...
  function _(e, T) {
    this.enc = e.encodingName, this.bomAware = e.bomAware, this.enc === "base64" ? this.encoder = I : this.enc === "utf8" ? this.encoder = i : this.enc === "cesu8" && (this.enc = "utf8", this.encoder = N, E.from("eda0bdedb2a9", "hex").toString() !== "💩" && (this.decoder = t, this.defaultCharUnicode = T.defaultCharUnicode));
  }
  _.prototype.encoder = n, _.prototype.decoder = R;
  var A = tI.StringDecoder;
  function R(e, T) {
    this.decoder = new A(T.enc);
  }
  R.prototype.write = function(e) {
    return E.isBuffer(e) || (e = E.from(e)), this.decoder.write(e);
  ...
  I.prototype.write = function(e) {
    ...
    return this.prevStr = e.slice(T), e = e.slice(0, T), E.from(e, "base64");
  ...
  N.prototype.write = function(e) {
    for (var T = E.alloc(e.length * 3), a = 0, u = 0; u < e.length; u++) {
      var S = e.charCodeAt(u);
      S < 128 ? T[a++] = S : S < 2048 ? (T[a++] = 192 + (S >>> 6), T[a++] = 128 + (S & 63)) : (T[a++] = 224 + (S >>> 12), T[a++] = 128 + (S >>> 6 & 63), T[a++] = 128 + (S & 63));
    }
    return T.slice(0, a);
  }, N.prototype.end = function() {
  };
  ...
  t.prototype.write = function(e) {
    for (var T = this.acc, a = this.contBytes, u = this.accBytes, S = "", O = 0; O < e.length; O++) {
      var D = e[O];
      (D & 192) !== 128 ? (a > 0 && (S += this.defaultCharUnicode, a = 0), D < 128 ? S += String.fromCharCode(D) : D < 224 ? (T = D & 31, a = 1, u = 1) : D < 240 ? (T = D & 15, a = 2, u = 1) : S += this.defaultCharUnicode) : a > 0 ? (T = T << 6 | D & 63, a--, u++, a === 0 && (u === 2 && T < 128 && T > 0 ? S += this.defaultCharUnicode : u === 3 && T < 2048 ? S += this.defaultCharUnicode : S += String.fromCharCode(T))) : S += this.defaultCharUnicode;
    }
    return this.acc = T, this.contBytes = a, this.accBytes = u, S;
  ...
  return i.prototype.write = function(e) {
    ...
    if (this.highSurrogate) {
      ...
      return this.highSurrogate = "", E.from(e, this.enc);
    }
  }, b_;
}
var BE = {}, xA;
function gI() {
  ...
  BE.utf32le = { type: "_utf32", isLE: !0 }, BE.utf32be = { type: "_utf32", isLE: !1 }, BE.ucs4le = "utf32le", BE.ucs4be = "utf32be", _.prototype.encoder = A, _.prototype.decoder = R;
  function A(e, T) {
    this.isLE = T.isLE, this.highSurrogate = 0;
  }
  A.prototype.write = function(e) {
    for (var T = E.from(e, "ucs2"), a = E.alloc(T.length * 2), u = this.isLE ? a.writeUInt32LE : a.writeUInt32BE, S = 0, O = 0; O < T.length; O += 2) {
      var D = T.readUInt16LE(O), c = D >= 55296 && D < 56320, h = D >= 56320 && D < 57344;
      if (this.highSurrogate)
        if (c || !h)
          u.call(a, this.highSurrogate, S), S += 4;
        else {
          var M = (this.highSurrogate - 55296 << 10 | D - 56320) + 65536;
          u.call(a, M, S), S += 4, this.highSurrogate = 0;
          continue;
        }
      c ? this.highSurrogate = D : (u.call(a, D, S), S += 4, this.highSurrogate = 0);
    }
    return S < a.length && (a = a.slice(0, S)), a;
  ...
  R.prototype.write = function(e) {
    if (e.length === 0)
      return "";
    var T = 0, a = 0, u = E.alloc(e.length + 4), S = 0, O = this.isLE, D = this.overflow, c = this.badChar;
    if (D.length > 0) {
      for (; T < e.length && D.length < 4; T++)
        D.push(e[T]);
      D.length === 4 && (O ? a = D[T] | D[T + 1] << 8 | D[T + 2] << 16 | D[T + 3] << 24 : a = D[T + 3] | D[T + 2] << 8 | D[T + 1] << 16 | D[T] << 24, D.length = 0, S = n(u, S, a, c));
    }
    for (; T < e.length - 3; T += 4)
      O ? a = e[T] | e[T + 1] << 8 | e[T + 2] << 16 | e[T + 3] << 24 : a = e[T + 3] | e[T + 2] << 8 | e[T + 1] << 16 | e[T] << 24, S = n(u, S, a, c);
    for (; T < e.length; T++)
      D.push(e[T]);
    return u.slice(0, S).toString("ucs2");
  };
  function n(e, T, a, u) {
    ...
    return e[T++] = a & 255, e[T++] = a >> 8, T;
  }
  R.prototype.end = function() {
    this.overflow.length = 0;
  }, BE.utf32 = I, BE.ucs4 = "utf32";
  function I(e, T) {
    this.iconv = T;
  }
  I.prototype.encoder = N, I.prototype.decoder = t;
  function N(e, T) {
    e = e || {}, e.addBOM === void 0 && (e.addBOM = !0), this.encoder = T.iconv.getEncoder(e.defaultEncoding || "utf-32le", e);
  }
  N.prototype.write = function(e) {
  ...
  t.prototype.write = function(e) {
    if (!this.decoder) {
      if (this.initialBufs.push(e), this.initialBufsLen += e.length, this.initialBufsLen < 32)
        return "";
    ...
    if (!this.decoder) {
      var e = i(this.initialBufs, this.options.defaultEncoding);
  ...
  function i(e, T) {
    var a = [], u = 0, S = 0, O = 0, D = 0, c = 0;
    E:
      for (var h = 0; h < e.length; h++)
        for (var M = e[h], L = 0; L < M.length; L++)
          if (a.push(M[L]), a.length === 4) {
            if (u === 0) {
            ...
            if ((a[0] !== 0 || a[1] > 16) && O++, (a[3] !== 0 || a[2] > 16) && S++, a[0] === 0 && a[1] === 0 && (a[2] !== 0 || a[3] !== 0) && c++, (a[0] !== 0 || a[1] !== 0) && a[2] === 0 && a[3] === 0 && D++, a.length = 0, u++, u >= 100)
              break E;
          }
    return c - O > D - S ? "utf-32be" : c - O < D - S ? "utf-32le" : T || "utf-32le";
  }
  return BE;
}
var e_ = {}, $A;
function lI() {
  ...
  _.prototype.encoder = A, _.prototype.decoder = R, _.prototype.bomAware = !0;
  function A() {
  }
  A.prototype.write = function(i) {
    for (var e = E.from(i, "ucs2"), T = 0; T < e.length; T += 2) {
      var a = e[T];
      e[T] = e[T + 1], e[T + 1] = a;
    }
    return e;
  ...
  R.prototype.write = function(i) {
    if (i.length == 0)
      return "";
    var e = E.alloc(i.length + 1), T = 0, a = 0;
    for (this.overflowByte !== -1 && (e[0] = i[0], e[1] = this.overflowByte, T = 1, a = 2); T < i.length - 1; T += 2, a += 2)
      e[a] = i[T + 1], e[a + 1] = i[T];
    return this.overflowByte = T == i.length - 1 ? i[i.length - 1] : -1, e.slice(0, a).toString("ucs2");
  }, R.prototype.end = function() {
    this.overflowByte = -1;
  }, e_.utf16 = n;
  function n(i, e) {
  ...
  n.prototype.encoder = I, n.prototype.decoder = N;
  function I(i, e) {
    i = i || {}, i.addBOM === void 0 && (i.addBOM = !0), this.encoder = e.iconv.getEncoder("utf-16le", i);
  }
  ...
  function N(i, e) {
    this.decoder = null, this.initialBufs = [], this.initialBufsLen = 0, this.options = i || {}, this.iconv = e.iconv;
  }
  N.prototype.write = function(i) {
    if (!this.decoder) {
      if (this.initialBufs.push(i), this.initialBufsLen += i.length, this.initialBufsLen < 16)
        return "";
      var e = t(this.initialBufs, this.options.defaultEncoding);
      this.decoder = this.iconv.getDecoder(e, this.options);
      for (var T = "", a = 0; a < this.initialBufs.length; a++)
        T += this.decoder.write(this.initialBufs[a]);
    ...
    if (!this.decoder) {
      var i = t(this.initialBufs, this.options.defaultEncoding);
  ...
  function t(i, e) {
    ...
    E:
      for (var O = 0; O < i.length; O++)
        for (var D = i[O], c = 0; c < D.length; c++)
          if (T.push(D[c]), T.length === 2) {
            if (a === 0) {
              if (T[0] === 255 && T[1] === 254) return "utf-16le";
              if (T[0] === 254 && T[1] === 255) return "utf-16be";
            }
            if (T[0] === 0 && T[1] !== 0 && S++, T[0] !== 0 && T[1] === 0 && u++, T.length = 0, a++, a >= 100)
              break E;
          }
    return S > u ? "utf-16be" : S < u ? "utf-16le" : e || "utf-16le";
  }
  return e_;
}
var kE = {}, ER;
function sI() {
  ...
  ER = 1;
  var E = wE.Buffer;
  kE.utf7 = _, kE.unicode11utf7 = "utf7";
  function _(D, c) {
    this.iconv = c;
  }
  _.prototype.encoder = R, _.prototype.decoder = n, _.prototype.bomAware = !0;
  var A = /[^A-Za-z0-9'\(\),-\.\/:\? \n\r\t]+/g;
  function R(D, c) {
    this.iconv = c.iconv;
  }
  R.prototype.write = function(D) {
    return E.from(D.replace(A, (function(c) {
  ...
  for (var I = /[A-Za-z0-9\/+]/, N = [], t = 0; t < 256; t++)
    N[t] = I.test(String.fromCharCode(t));
  var i = 43, e = 45, T = 38;
  n.prototype.write = function(D) {
    for (var c = "", h = 0, M = this.inBase64, L = this.base64Accum, B = 0; B < D.length; B++)
      if (!M)
        D[B] == i && (c += this.iconv.decode(D.slice(h, B), "ascii"), h = B + 1, M = !0);
      else if (!N[D[B]]) {
        if (B == h && D[B] == e)
          c += "+";
        else {
          var s = L + this.iconv.decode(D.slice(h, B), "ascii");
          c += this.iconv.decode(E.from(s, "base64"), "utf16-be");
        }
        D[B] != e && B--, h = B + 1, M = !1, L = "";
      }
    if (!M)
      c += this.iconv.decode(D.slice(h), "ascii");
    else {
      var s = L + this.iconv.decode(D.slice(h), "ascii"), l = s.length - s.length % 8;
      L = s.slice(l), s = s.slice(0, l), c += this.iconv.decode(E.from(s, "base64"), "utf16-be");
    }
    return this.inBase64 = M, this.base64Accum = L, c;
  }, n.prototype.end = function() {
    var D = "";
  ...
  a.prototype.encoder = u, a.prototype.decoder = S, a.prototype.bomAware = !0;
  function u(D, c) {
    this.iconv = c.iconv, this.inBase64 = !1, this.base64Accum = E.alloc(6), this.base64AccumIdx = 0;
  }
  u.prototype.write = function(D) {
    for (var c = this.inBase64, h = this.base64Accum, M = this.base64AccumIdx, L = E.alloc(D.length * 5 + 10), B = 0, s = 0; s < D.length; s++) {
      var l = D.charCodeAt(s);
      l >= 32 && l <= 126 ? (c && (M > 0 && (B += L.write(h.slice(0, M).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), B), M = 0), L[B++] = e, c = !1), c || (L[B++] = l, l === T && (L[B++] = e))) : (c || (L[B++] = T, c = !0), c && (h[M++] = l >> 8, h[M++] = l & 255, M == h.length && (B += L.write(h.toString("base64").replace(/\//g, ","), B), M = 0)));
    }
    return this.inBase64 = c, this.base64AccumIdx = M, L.slice(0, B);
  }, u.prototype.end = function() {
    var D = E.alloc(10), c = 0;
    return this.inBase64 && (this.base64AccumIdx > 0 && (c += D.write(this.base64Accum.slice(0, this.base64AccumIdx).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), c), this.base64AccumIdx = 0), D[c++] = e, this.inBase64 = !1), D.slice(0, c);
  };
  function S(D, c) {
    this.iconv = c.iconv, this.inBase64 = !1, this.base64Accum = "";
  }
  var O = N.slice();
  return O[44] = !0, S.prototype.write = function(D) {
    for (var c = "", h = 0, M = this.inBase64, L = this.base64Accum, B = 0; B < D.length; B++)
      if (!M)
        D[B] == T && (c += this.iconv.decode(D.slice(h, B), "ascii"), h = B + 1, M = !0);
      else if (!O[D[B]]) {
        if (B == h && D[B] == e)
          c += "&";
        else {
          var s = L + this.iconv.decode(D.slice(h, B), "ascii").replace(/,/g, "/");
          c += this.iconv.decode(E.from(s, "base64"), "utf16-be");
        }
        D[B] != e && B--, h = B + 1, M = !1, L = "";
      }
    if (!M)
      ...
      var s = L + this.iconv.decode(D.slice(h), "ascii").replace(/,/g, "/"), l = s.length - s.length % 8;
      L = s.slice(l), s = s.slice(0, l), c += this.iconv.decode(E.from(s, "base64"), "utf16-be");
    }
    return this.inBase64 = M, this.base64Accum = L, c;
...
function fI() {
  ...
  function _(n, I) {
    ...
    if (!n.chars || n.chars.length !== 128 && n.chars.length !== 256)
      throw new Error("Encoding '" + n.type + "' has incorrect 'chars' (must be of len 128 or 256)");
    if (n.chars.length === 128) {
      for (var N = "", t = 0; t < 128; t++)
        N += String.fromCharCode(t);
      n.chars = N + n.chars;
    }
    this.decodeBuf = E.from(n.chars, "ucs2");
    for (var i = E.alloc(65536, I.defaultCharSingleByte.charCodeAt(0)), t = 0; t < n.chars.length; t++)
      i[n.chars.charCodeAt(t)] = t;
    this.encodeBuf = i;
  }
  _.prototype.encoder = A, _.prototype.decoder = R;
  function A(n, I) {
    this.encodeBuf = I.encodeBuf;
  }
  A.prototype.write = function(n) {
    for (var I = E.alloc(n.length), N = 0; N < n.length; N++)
      I[N] = this.encodeBuf[n.charCodeAt(N)];
    return I;
  }, A.prototype.end = function() {
  };
  function R(n, I) {
  ...
  return R.prototype.write = function(n) {
    for (var I = this.decodeBuf, N = E.alloc(n.length * 2), t = 0, i = 0, e = 0; e < n.length; e++)
      t = n[e] * 2, i = e * 2, N[i] = I[t], N[i + 1] = I[t + 1];
...
function GI() {
  ...
  var E = wE.Buffer;
  W_._dbcs = i;
  for (var _ = -1, A = -2, R = -10, n = -1e3, I = new Array(256), N = -1, t = 0; t < 256; t++)
    I[t] = _;
  function i(u, S) {
    ...
    if (typeof u.gb18030 == "function") {
      ...
      this.decodeTables.push(I.slice(0));
      for (var M = this.decodeTables[0], D = 129; D <= 254; D++)
        for (var L = this.decodeTables[n - M[D]], B = 48; B <= 57; B++) {
          if (L[B] === _)
            L[B] = n - c;
          else if (L[B] > n)
            throw new Error("gb18030 decode tables conflict at byte 2");
          for (var s = this.decodeTables[n - L[B]], l = 129; l <= 254; l++) {
            if (s[l] === _)
              s[l] = n - h;
            else {
              if (s[l] === n - h)
            ...
            for (var d = this.decodeTables[n - s[l]], o = 48; o <= 57; o++)
              d[o] === _ && (d[o] = A);
          }
        }
    }
    ...
    if (u.encodeSkipVals)
      for (var D = 0; D < u.encodeSkipVals.length; D++) {
        var RE = u.encodeSkipVals[D];
        if (typeof RE == "number")
          Z[RE] = !0;
        else
          for (var B = RE.from; B <= RE.to; B++)
            Z[B] = !0;
      }
    if (this._fillEncodeTable(0, 0, Z), u.encodeAdd)
      for (var OE in u.encodeAdd)
        Object.prototype.hasOwnProperty.call(u.encodeAdd, OE) && this._setEncodeChar(OE.charCodeAt(0), u.encodeAdd[OE]);
    this.defCharSB = this.encodeTable[0][S.defaultCharSingleByte.charCodeAt(0)], this.defCharSB === _ && (this.defCharSB = this.encodeTable[0]["?"]), this.defCharSB === _ && (this.defCharSB = 63);
  }
  i.prototype.encoder = e, i.prototype.decoder = T, i.prototype._getDecodeTrieNode = function(u) {
    for (var S = []; u > 0; u >>>= 8)
      S.push(u & 255);
    S.length == 0 && S.push(0);
    for (var O = this.decodeTables[0], D = S.length - 1; D > 0; D--) {
      var c = O[S[D]];
      if (c == _)
        O[S[D]] = n - this.decodeTables.length, this.decodeTables.push(O = I.slice(0));
      else if (c <= n)
        O = this.decodeTables[n - c];
      else
        throw new Error("Overwrite byte in " + this.encodingName + ", addr: " + u.toString(16));
    }
    return O;
  }, i.prototype._addDecodeChunk = function(u) {
    var S = parseInt(u[0], 16), O = this._getDecodeTrieNode(S);
    S = S & 255;
      ...
      if (typeof c == "string")
        for (var h = 0; h < c.length; ) {
          ...
          if (M >= 55296 && M < 56320) {
            ...
            if (L >= 56320 && L < 57344)
              O[S++] = 65536 + (M - 55296) * 1024 + (L - 56320);
            else
              throw new Error("Incorrect surrogate pair in " + this.encodingName + " at chunk " + u[0]);
          } else if (M > 4080 && M <= 4095) {
            for (var B = 4095 - M + 2, s = [], l = 0; l < B; l++)
              s.push(c.charCodeAt(h++));
            O[S++] = R - this.decodeTableSeq.length, this.decodeTableSeq.push(s);
          } else
            O[S++] = M;
        }
      else if (typeof c == "number")
        for (var d = O[S - 1] + 1, h = 0; h < c; h++)
          O[S++] = d++;
      else
        throw new Error("Incorrect type '" + typeof c + "' given in " + this.encodingName + " at chunk " + u[0]);
    }
    if (S > 255)
      throw new Error("Incorrect chunk in " + this.encodingName + " at addr " + u[0] + ": too long" + S);
  }, i.prototype._getEncodeBucket = function(u) {
    var S = u >> 8;
    return this.encodeTable[S] === void 0 && (this.encodeTable[S] = I.slice(0)), this.encodeTable[S];
  }, i.prototype._setEncodeChar = function(u, S) {
    var O = this._getEncodeBucket(u), D = u & 255;
    O[D] <= R ? this.encodeTableSeq[R - O[D]][N] = S : O[D] == _ && (O[D] = S);
  }, i.prototype._setEncodeSequence = function(u, S) {
    var O = u[0], D = this._getEncodeBucket(O), c = O & 255, h;
    D[c] <= R ? h = this.encodeTableSeq[R - D[c]] : (h = {}, D[c] !== _ && (h[N] = D[c]), D[c] = R - this.encodeTableSeq.length, this.encodeTableSeq.push(h));
    for (var M = 1; M < u.length - 1; M++) {
      var L = h[O];
      typeof L == "object" ? h = L : (h = h[O] = {}, L !== void 0 && (h[N] = L));
    }
    O = u[u.length - 1], h[O] = S;
  }, i.prototype._fillEncodeTable = function(u, S, O) {
    for (var D = this.decodeTables[u], c = !1, h = {}, M = 0; M < 256; M++) {
      var L = D[M], B = S + M;
      if (!O[B])
        if (L >= 0)
          this._setEncodeChar(L, B), c = !0;
        else if (L <= n) {
          var s = n - L;
          if (!h[s]) {
            var l = B << 8 >>> 0;
            this._fillEncodeTable(s, l, O) ? c = !0 : h[s] = !0;
          }
        } else L <= R && (this._setEncodeSequence(this.decodeTableSeq[R - L], B), c = !0);
    }
    return c;
  };
  function e(u, S) {
  ...
  e.prototype.write = function(u) {
    for (var S = E.alloc(u.length * (this.gb18030 ? 4 : 3)), O = this.leadSurrogate, D = this.seqObj, c = -1, h = 0, M = 0; ; ) {
      if (c === -1) {
        if (h == u.length) break;
        var L = u.charCodeAt(h++);
      } else {
        var L = c;
        c = -1;
      }
      if (L >= 55296 && L < 57344)
        if (L < 56320)
          ...
          O !== -1 ? (L = 65536 + (O - 55296) * 1024 + (L - 56320), O = -1) : L = _;
      else O !== -1 && (c = L, L = _, O = -1);
      var B = _;
      if (D !== void 0 && L != _) {
        var s = D[L];
        if (typeof s == "object") {
          D = s;
          continue;
        } else typeof s == "number" ? B = s : s == null && (s = D[N], s !== void 0 && (B = s, c = L));
        D = void 0;
      } else if (L >= 0) {
        var l = this.encodeTable[L >> 8];
        if (l !== void 0 && (B = l[L & 255]), B <= R) {
          D = this.encodeTableSeq[R - B];
          continue;
        }
        if (B == _ && this.gb18030) {
          var d = a(this.gb18030.uChars, L);
          if (d != -1) {
            var B = this.gb18030.gbChars[d] + (L - this.gb18030.uChars[d]);
            S[M++] = 129 + Math.floor(B / 12600), B = B % 12600, S[M++] = 48 + Math.floor(B / 1260), B = B % 1260, S[M++] = 129 + Math.floor(B / 10), B = B % 10, S[M++] = 48 + B;
            continue;
          }
        }
      }
      B === _ && (B = this.defaultCharSingleByte), B < 256 ? S[M++] = B : B < 65536 ? (S[M++] = B >> 8, S[M++] = B & 255) : B < 16777216 ? (S[M++] = B >> 16, S[M++] = B >> 8 & 255, S[M++] = B & 255) : (S[M++] = B >>> 24, S[M++] = B >>> 16 & 255, S[M++] = B >>> 8 & 255, S[M++] = B & 255);
    }
    return this.seqObj = D, this.leadSurrogate = O, S.slice(0, M);
  }, e.prototype.end = function() {
    if (!(this.leadSurrogate === -1 && this.seqObj === void 0)) {
      var u = E.alloc(10), S = 0;
      if (this.seqObj) {
        var O = this.seqObj[N];
        O !== void 0 && (O < 256 ? u[S++] = O : (u[S++] = O >> 8, u[S++] = O & 255)), this.seqObj = void 0;
      }
      return this.leadSurrogate !== -1 && (u[S++] = this.defaultCharSingleByte, this.leadSurrogate = -1), u.slice(0, S);
    }
  }, e.prototype.findIdx = a;
  function T(u, S) {
  ...
  T.prototype.write = function(u) {
    for (var S = E.alloc(u.length * 2), O = this.nodeIdx, D = this.prevBytes, c = this.prevBytes.length, h = -this.prevBytes.length, M, L = 0, B = 0; L < u.length; L++) {
      var s = L >= 0 ? u[L] : D[L + c], M = this.decodeTables[O][s];
      if (!(M >= 0)) if (M === _)
        M = this.defaultCharUnicode.charCodeAt(0), L = h;
      else if (M === A) {
        if (L >= 3)
          var l = (u[L - 3] - 129) * 12600 + (u[L - 2] - 48) * 1260 + (u[L - 1] - 129) * 10 + (s - 48);
        else
          var l = (D[L - 3 + c] - 129) * 12600 + ((L - 2 >= 0 ? u[L - 2] : D[L - 2 + c]) - 48) * 1260 + ((L - 1 >= 0 ? u[L - 1] : D[L - 1 + c]) - 129) * 10 + (s - 48);
        var d = a(this.gb18030.gbChars, l);
        M = this.gb18030.uChars[d] + l - this.gb18030.gbChars[d];
      } else if (M <= n) {
        O = n - M;
        continue;
      } else if (M <= R) {
        for (var o = this.decodeTableSeq[R - M], Z = 0; Z < o.length - 1; Z++)
          M = o[Z], S[B++] = M & 255, S[B++] = M >> 8;
        M = o[o.length - 1];
      } else
        throw new Error("iconv-lite internal error: invalid decoding table value " + M + " at " + O + "/" + s);
      if (M >= 65536) {
        M -= 65536;
        var RE = 55296 | M >> 10;
        S[B++] = RE & 255, S[B++] = RE >> 8, M = 56320 | M & 1023;
      }
      S[B++] = M & 255, S[B++] = M >> 8, O = 0, h = L + 1;
    }
    return this.nodeIdx = O, this.prevBytes = h >= 0 ? Array.prototype.slice.call(u, h) : D.slice(h + c).concat(Array.prototype.slice.call(u)), S.slice(0, B).toString("ucs2");
  }, T.prototype.end = function() {
    for (var u = ""; this.prevBytes.length > 0; ) {
      u += this.defaultCharUnicode;
      var S = this.prevBytes.slice(1);
      this.prevBytes = [], this.nodeIdx = 0, S.length > 0 && (u += this.write(S));
    }
...
function WI() {
  ...
  tR = 1;
  var E = wE.Buffer;
  return v_ = function(_) {
    var A = _.Transform;
    function R(I, N) {
      this.conv = I, N = N || {}, N.decodeStrings = !1, A.call(this, N);
    }
    R.prototype = Object.create(A.prototype, {
      constructor: { value: R }
    }), R.prototype._transform = function(I, N, t) {
      if (typeof I != "string")
        return t(new Error("Iconv encoding stream needs strings as its input."));
      try {
        var i = this.conv.write(I);
        i && i.length && this.push(i), t();
      } catch (e) {
        t(e);
      }
      ...
      var N = [];
      return this.on("error", I), this.on("data", function(t) {
        N.push(t);
    ...
    function n(I, N) {
      this.conv = I, N = N || {}, N.encoding = this.encoding = "utf8", A.call(this, N);
    }
    return n.prototype = Object.create(A.prototype, {
      constructor: { value: n }
    }), n.prototype._transform = function(I, N, t) {
      if (!E.isBuffer(I) && !(I instanceof Uint8Array))
        return t(new Error("Iconv decoding stream needs buffers as its input."));
      try {
        var i = this.conv.write(I);
        i && i.length && this.push(i, this.encoding), t();
      } catch (e) {
        t(e);
      }
      ...
      IconvLiteDecoderStream: n
    };
  }, v_;
}
QA.exports;
(function(E) {
  var _ = wE.Buffer, A = PA, R = Mn;
  E.exports.encodings = null, E.exports.defaultCharUnicode = "�", E.exports.defaultCharSingleByte = "?", E.exports.encode = function(N, t, i) {
    N = "" + (N || "");
    var e = E.exports.getEncoder(t, i), T = e.write(N), a = e.end();
    return a && a.length > 0 ? _.concat([T, a]) : T;
  }, E.exports.decode = function(N, t, i) {
    typeof N == "string" && (E.exports.skipDecodeWarning || (console.error("Iconv-lite warning: decode()-ing strings is deprecated. Refer to https://github.com/ashtuchkin/iconv-lite/wiki/Use-Buffers-when-decoding"), E.exports.skipDecodeWarning = !0), N = _.from("" + (N || ""), "binary"));
    var e = E.exports.getDecoder(t, i), T = e.write(N), a = e.end();
    return a ? T + a : T;
    ...
    if (!E.exports.encodings) {
      var t = mI();
      E.exports.encodings = { __proto__: null }, R(E.exports.encodings, t);
    }
    for (var i = E.exports._canonicalizeEncoding(N), e = {}; ; ) {
      ...
      switch (typeof a) {
        ...
        case "object":
          for (var u in a)
            e[u] = a[u];
          e.encodingName || (e.encodingName = i), i = a.type;
          break;
        case "function":
          return e.encodingName || (e.encodingName = i), T = new a(e, E.exports), E.exports._codecDataCache[e.encodingName] = T, T;
        default:
          throw new Error("Encoding not recognized: '" + N + "' (searched as: '" + i + "')");
      }
    }
  }, E.exports._canonicalizeEncoding = function(I) {
    return ("" + I).toLowerCase().replace(/:\d{4}$|[^0-9a-z]/g, "");
  }, E.exports.getEncoder = function(N, t) {
    var i = E.exports.getCodec(N), e = new i.encoder(t, i);
    return i.bomAware && t && t.addBOM && (e = new A.PrependBOM(e, t)), e;
  }, E.exports.getDecoder = function(N, t) {
    var i = E.exports.getCodec(N), e = new i.decoder(t, i);
    return i.bomAware && !(t && t.stripBOM === !1) && (e = new A.StripBOM(e, t)), e;
  }, E.exports.enableStreamingAPI = function(N) {
    if (!E.exports.supportsStreams) {
      var t = WI()(N);
      E.exports.IconvLiteEncoderStream = t.IconvLiteEncoderStream, E.exports.IconvLiteDecoderStream = t.IconvLiteDecoderStream, E.exports.encodeStream = function(e, T) {
        return new E.exports.IconvLiteEncoderStream(E.exports.getEncoder(e, T), T);
      }, E.exports.decodeStream = function(e, T) {
        return new E.exports.IconvLiteDecoderStream(E.exports.getDecoder(e, T), T);
  ...
  n && n.Transform ? E.exports.enableStreamingAPI(n) : E.exports.encodeStream = E.exports.decodeStream = function() {
    throw new Error("iconv-lite Streaming API is not enabled. Use iconv.enableStreamingAPI(require('stream')); to enable it.");
  };
})(QA);
var HI = QA.exports;
const BA = HI, { createLRU: yI } = oE, t_ = yI({
  max: 500
});
zE.decode = function(E, _, A, R, n) {
  if (Buffer.isEncoding(_))
    return E.toString(_, A, R);
  let I;
  if (!n)
    I = t_.get(_), I || (I = BA.getDecoder(_), t_.set(_, I));
  else {
    const i = { encoding: _, options: n }, e = JSON.stringify(i);
    I = t_.get(e), I || (I = BA.getDecoder(i.encoding, i.options), t_.set(e, I));
  }
  const N = I.write(E.slice(A, R)), t = I.end();
  return t ? N + t : N;
};
zE.encode = function(E, _, A) {
  if (Buffer.isEncoding(_))
    return Buffer.from(E, _);
  const R = BA.getEncoder(_, A || {}), n = R.write(E), I = R.end();
  return I && I.length > 0 ? Buffer.concat([n, I]) : n;
};
var Q = { exports: {} }, iR;
function UE() {
  return iR || (iR = 1, Q.exports = {
    0: "DECIMAL",
    // aka DECIMAL
    1: "TINY",
    // aka TINYINT, 1 byte
    ...
    255: "GEOMETRY"
    // aka GEOMETRY
  }, Q.exports.DECIMAL = 0, Q.exports.TINY = 1, Q.exports.SHORT = 2, Q.exports.LONG = 3, Q.exports.FLOAT = 4, Q.exports.DOUBLE = 5, Q.exports.NULL = 6, Q.exports.TIMESTAMP = 7, Q.exports.LONGLONG = 8, Q.exports.INT24 = 9, Q.exports.DATE = 10, Q.exports.TIME = 11, Q.exports.DATETIME = 12, Q.exports.YEAR = 13, Q.exports.NEWDATE = 14, Q.exports.VARCHAR = 15, Q.exports.BIT = 16, Q.exports.VECTOR = 242, Q.exports.JSON = 245, Q.exports.NEWDECIMAL = 246, Q.exports.ENUM = 247, Q.exports.SET = 248, Q.exports.TINY_BLOB = 249, Q.exports.MEDIUM_BLOB = 250, Q.exports.LONG_BLOB = 251, Q.exports.BLOB = 252, Q.exports.VAR_STRING = 253, Q.exports.STRING = 254, Q.exports.GEOMETRY = 255), Q.exports;
}
const vI = FA, XI = On.Buffer, FE = MI, QE = zE, KI = UE(), jI = /* @__PURE__ */ new Date(NaN), JI = "000000000000";
function IE(E, _) {
  const A = _.toString();
  return A.length >= E ? A : (JI + A).slice(-E);
}
const X_ = 45, K_ = 43, zI = 46, ZI = 101, kI = 69;
let qI = class MA {
  constructor(_, A, R, n) {
    this.sequenceId = _, this.numPackets = 1, this.buffer = A, this.start = R, this.offset = R + 4, this.end = n;
  }
  // ==============================
  // readers
  // ==============================
  ...
  readInt64JSNumber() {
    const _ = this.readInt32(), A = this.readInt32();
    return new FE(_, A, !0).toNumber();
  }
  readSInt64JSNumber() {
    const _ = this.readInt32(), A = this.readInt32();
    return A & 2147483648 ? new FE(_, A, !1).toNumber() : _ + 4294967296 * A;
  }
  readInt64String() {
    const _ = this.readInt32(), A = this.readInt32();
    return new FE(_, A, !0).toString();
  }
  readSInt64String() {
    const _ = this.readInt32(), A = this.readInt32();
    return new FE(_, A, !1).toString();
  }
  readInt64() {
    const _ = this.readInt32(), A = this.readInt32();
    let R = new FE(_, A, !0);
    const n = R.toNumber(), I = R.toString();
    return R = n.toString() === I ? n : I, R;
  }
  readSInt64() {
    const _ = this.readInt32(), A = this.readInt32();
    let R = new FE(_, A, !1);
  ...
  readLengthCodedNumber(_, A) {
    const R = this.buffer[this.offset++];
    return R < 251 ? R : this.readLengthCodedNumberExt(R, _, A);
  }
  readLengthCodedNumberSigned(_) {
    return this.readLengthCodedNumber(_, !0);
  }
  readLengthCodedNumberExt(_, A, R) {
    let n, I, N;
    if (_ === 251)
      return null;
    if (_ === 252)
    ...
    if (_ === 254) {
      if (n = this.readInt32(), I = this.readInt32(), I === 0)
        return n;
      if (I < 2097152)
        return I * 4294967296 + n;
      N = new FE(n, I, !R);
      const t = N.toNumber(), i = N.toString();
      return N = t.toString() === i ? t : i, A ? i : N;
    }
  ...
  readDateTime(_) {
    if (!_ || _ === "Z" || _ === "local") {
      ...
      if (R === 251)
        return null;
      let n = 0, I = 0, N = 0, t = 0, i = 0, e = 0, T = 0;
      return R > 3 && (n = this.readInt16(), I = this.readInt8(), N = this.readInt8()), R > 6 && (t = this.readInt8(), i = this.readInt8(), e = this.readInt8()), R > 10 && (T = this.readInt32() / 1e3), n + I + N + t + i + e + T === 0 ? jI : _ === "Z" ? new Date(Date.UTC(n, I - 1, N, t, i, e, T)) : new Date(n, I - 1, N, t, i, e, T);
    }
    let A = this.readDateTimeString(6, "T", null);
    return A.length === 10 && (A += "T00:00:00"), new Date(A + _);
  }
  readDateTimeString(_, A, R) {
    const n = this.readInt8();
    let I = 0, N = 0, t = 0, i = 0, e = 0, T = 0, a = 0, u;
    return n > 3 && (I = this.readInt16(), N = this.readInt8(), t = this.readInt8(), u = [IE(4, I), IE(2, N), IE(2, t)].join("-")), n > 6 ? (i = this.readInt8(), e = this.readInt8(), T = this.readInt8(), u += `${A || " "}${[
      IE(2, i),
      IE(2, e),
      IE(2, T)
    ].join(":")}`) : R === KI.DATETIME && (u += " 00:00:00"), n > 10 && (a = this.readInt32(), u += ".", _ && (a = IE(6, a), a.length > _ && (a = a.substring(0, _))), u += a), u;
  }
  ...
  readTimeString(_) {
    ...
    let n = 0, I = 0, N = 0, t = 0, i = 0;
    return A > 6 && (n = this.readInt32(), I = this.readInt8(), N = this.readInt8(), t = this.readInt8()), A > 10 && (i = this.readInt32()), _ ? (I += n * 24, N += I * 60, t += N * 60, i += t * 1e3, i *= R, i) : (R === -1 ? "-" : "") + [IE(2, n * 24 + I), IE(2, N), IE(2, t)].join(":") + (i ? `.${i}`.replace(/0+$/, "") : "");
  }
  readLengthCodedString(_) {
    const A = this.readLengthCodedNumber();
    return A === null ? null : (this.offset += A, QE.decode(
      this.buffer,
      _,
      this.offset - A,
      this.offset
  ...
  readNullTerminatedString(_) {
    ...
    for (; this.buffer[R]; )
      R = R + 1;
    return this.offset = R + 1, QE.decode(this.buffer, _, A, R);
  }
  // TODO reuse?
  readString(_, A) {
    return typeof _ == "string" && typeof A > "u" && (A = _, _ = void 0), typeof _ > "u" && (_ = this.end - this.offset), this.offset += _, QE.decode(
      this.buffer,
      A,
      this.offset - _,
      this.offset
  ...
  parseInt(_, A) {
    ...
    let R = 0;
    const n = this.offset, I = this.offset + _;
    let N = 1;
    if (_ === 0)
      return 0;
    this.buffer[this.offset] === X_ && (this.offset++, N = -1);
    let t;
    const i = I - this.offset;
    if (A) {
      if (i >= 15)
        return t = this.readString(I - this.offset, "binary"), R = parseInt(t, 10), R.toString() === t ? N * R : N === -1 ? `-${t}` : t;
      if (i > 16)
        return t = this.readString(I - this.offset), N === -1 ? `-${t}` : t;
    }
    for (this.buffer[this.offset] === K_ && this.offset++; this.offset < I; )
      R *= 10, R += this.buffer[this.offset] - 48, this.offset++;
    const e = R * N;
    return !A || (t = this.buffer.toString("ascii", n, I), e.toString() === t) ? e : t;
  }
  ...
  parseIntNoBigCheck(_) {
    if (_ === null)
      return null;
    let A = 0;
    const R = this.offset + _;
    let n = 1;
    if (_ === 0)
      return 0;
    for (this.buffer[this.offset] === X_ && (this.offset++, n = -1), this.buffer[this.offset] === K_ && this.offset++; this.offset < R; )
  ...
  parseGeometryValue() {
    ...
    if (_ === null || !_.length)
      return null;
    function R() {
      let n, I, N, t, i, e, T = null;
      const a = _.readUInt8(A);
      A += 1;
      const u = a ? _.readUInt32LE(A) : _.readUInt32BE(A);
      switch (A += 4, u) {
        case 1:
          n = a ? _.readDoubleLE(A) : _.readDoubleBE(A), A += 8, I = a ? _.readDoubleLE(A) : _.readDoubleBE(A), A += 8, T = { x: n, y: I };
          break;
        case 2:
          for (i = a ? _.readUInt32LE(A) : _.readUInt32BE(A), A += 4, T = [], N = i; N > 0; N--)
            n = a ? _.readDoubleLE(A) : _.readDoubleBE(A), A += 8, I = a ? _.readDoubleLE(A) : _.readDoubleBE(A), A += 8, T.push({ x: n, y: I });
          break;
        case 3:
          const S = a ? _.readUInt32LE(A) : _.readUInt32BE(A);
          for (A += 4, T = [], N = S; N > 0; N--) {
            for (i = a ? _.readUInt32LE(A) : _.readUInt32BE(A), A += 4, e = [], t = i; t > 0; t--)
              n = a ? _.readDoubleLE(A) : _.readDoubleBE(A), A += 8, I = a ? _.readDoubleLE(A) : _.readDoubleBE(A), A += 8, e.push({ x: n, y: I });
            T.push(e);
          }
          break;
        case 4:
        ...
        case 6:
        case 7:
          const O = a ? _.readUInt32LE(A) : _.readUInt32BE(A);
          for (A += 4, T = [], N = O; N > 0; N--)
            T.push(R());
          break;
      }
  ...
  parseVector() {
    const _ = this.readLengthCodedNumber(), A = this.offset + _, R = [];
  ...
  parseDate(_) {
    ...
    this.offset++;
    const n = this.parseInt(2);
    this.offset++;
    const I = this.parseInt(2);
    return !_ || _ === "local" ? new Date(R, n - 1, I) : _ === "Z" ? new Date(Date.UTC(R, n - 1, I)) : /* @__PURE__ */ new Date(
      `${IE(4, R)}-${IE(2, n)}-${IE(2, I)}T00:00:00${_}`
    );
  }
  parseDateTime(_) {
    const A = this.readLengthCodedString("binary");
    return A === null ? null : !_ || _ === "local" ? new Date(A) : /* @__PURE__ */ new Date(`${A}${_}`);
  }
  parseFloat(_) {
    if (_ === null)
      return null;
    ...
    if (_ === 0)
      return 0;
    for (this.buffer[this.offset] === X_ && (this.offset++, n = -1), this.buffer[this.offset] === K_ && this.offset++; this.offset < R; )
      if (N = this.buffer[this.offset], N === zI)
        I = !0, this.offset++;
      else if (N === ZI || N === kI) {
        this.offset++;
        const t = this.parseInt(R - this.offset);
        return A / n * Math.pow(10, t);
      } else
        A *= 10, A += this.buffer[this.offset] - 48, this.offset++, I && (n = n * 10);
    return A / n;
  }
  ...
  asError(_) {
    ...
    const n = this.readString(void 0, _), I = new Error(n);
    return I.code = vI[A], I.errno = A, I.sqlState = R, I.sqlMessage = n, I;
  }
  writeInt32(_) {
  ...
  writeNullTerminatedString(_, A) {
    const R = QE.encode(_, A);
  ...
  writeDate(_, A) {
    if (this.buffer.writeUInt8(11, this.offset), !A || A === "local")
      ...
      if (A !== "Z") {
        const R = (A[0] === "-" ? -1 : 1) * (parseInt(A.substring(1, 3), 10) * 60 + parseInt(A.substring(4), 10));
        R !== 0 && (_ = new Date(_.getTime() + 6e4 * R));
      }
  ...
  writeHeader(_) {
    const A = this.offset;
    this.offset = 0, this.writeInt24(this.buffer.length - 4), this.writeInt8(_), this.offset = A;
  }
  clone() {
    return new MA(this.sequenceId, this.buffer, this.start, this.end);
  }
  type() {
  ...
  static lengthCodedStringLength(_, A) {
    const n = QE.encode(_, A).length;
    return MA.lengthCodedNumberLength(n) + n;
  }
  static MockBuffer() {
    const _ = function() {
    }, A = Buffer.alloc(0);
    for (const R in XI.prototype)
      typeof A[R] == "function" && (A[R] = _);
    return A;
  }
};
var K = qI;
const i_ = K, a_ = 16777215;
function pI(E, _) {
  const A = E[_], R = E[_ + 1], n = E[_ + 2];
  return R + n === 0 ? A : A + (R << 8) + (n << 16);
}
let xI = class ME {
  constructor(_, A) {
    typeof A > "u" && (A = 4), this.buffer = [], this.bufferLength = 0, this.packetHeaderLength = A, this.headerLen = 0, this.length = 0, this.largePacketParts = [], this.firstPacketSequenceId = 0, this.onPacket = _, this.execute = ME.prototype.executeStart, this._flushLargePacket = A === 7 ? this._flushLargePacket7 : this._flushLargePacket4;
  }
  _flushLargePacket4() {
    const _ = this.largePacketParts.length;
    this.largePacketParts.unshift(Buffer.from([0, 0, 0, 0]));
    const A = Buffer.concat(this.largePacketParts), R = new i_(this.firstPacketSequenceId, A, 0, A.length);
    this.largePacketParts.length = 0, R.numPackets = _, this.onPacket(R);
  }
  _flushLargePacket7() {
    const _ = this.largePacketParts.length;
    ...
    const R = new i_(this.firstPacketSequenceId, A, 0, A.length);
    R.numPackets = _, this.onPacket(R);
  }
  executeStart(_) {
    let A = 0;
    const R = _.length;
    for (; R - A >= 3; )
      if (this.length = pI(_, A), R - A >= this.length + this.packetHeaderLength) {
        const n = _[A + 3];
        this.length < a_ && this.largePacketParts.length === 0 ? this.onPacket(
          new i_(
            n,
            _,
            A,
        ...
        ), this.length < a_ && this._flushLargePacket()), A += this.packetHeaderLength + this.length;
      } else {
        this.buffer = [_.slice(A + 3, R)], this.bufferLength = R - A - 3, this.execute = ME.prototype.executePayload;
        return;
      }
    R - A > 0 && (this.headerLen = R - A, this.length = _[A], this.headerLen === 2 ? (this.length = _[A] + (_[A + 1] << 8), this.execute = ME.prototype.executeHeader3) : this.execute = ME.prototype.executeHeader2);
  }
  executePayload(_) {
    let A = 0;
    const R = _.length, n = this.length - this.bufferLength + this.packetHeaderLength - 3;
    if (R - A >= n) {
      const I = Buffer.allocUnsafe(this.length + this.packetHeaderLength);
      let N = 3;
      for (let i = 0; i < this.buffer.length; ++i)
        this.buffer[i].copy(I, N), N += this.buffer[i].length;
      _.copy(I, N, A, A + n);
      const t = I[3];
      if (this.length < a_ && this.largePacketParts.length === 0 ? this.onPacket(
        new i_(
          t,
          I,
          0,
      ...
      ), this.length < a_ && this._flushLargePacket()), this.buffer = [], this.bufferLength = 0, this.execute = ME.prototype.executeStart, A += n, R - A > 0)
        return this.execute(_.slice(A, R));
    } else
...
class oA {
  ...
  toPacket(_) {
    const A = 6 + this.pluginName.length + this.pluginData.length, R = Buffer.allocUnsafe(A), n = new $I(0, R, 0, A);
  ...
  static fromPacket(_, A) {
    _.readInt8();
    const R = _.readNullTerminatedString(A), n = _.readBuffer();
    return new oA({
      pluginName: R,
      pluginData: n
    });
...
class YA {
  ...
  toPacket() {
    const _ = 6 + this.pluginName.length + this.pluginData.length, A = Buffer.allocUnsafe(_), R = new _N(0, A, 0, _);
    return R.offset = 4, R.writeInt8(254), R.writeNullTerminatedString(this.pluginName, "cesu8"), R.writeBuffer(this.pluginData), R;
  }
  static fromPacket(_) {
    _.readInt8();
    const A = _.readNullTerminatedString("cesu8"), R = _.readBuffer();
    return new YA({
      pluginName: A,
      pluginData: R
    });
...
class mA {
  ...
  toPacket() {
    const _ = 5 + this.data.length, A = Buffer.allocUnsafe(_), R = new RN(0, A, 0, _);
    return R.offset = 4, R.writeInt8(1), R.writeBuffer(this.data), R;
  }
  static fromPacket(_) {
    _.readInt8();
...
class WA {
  ...
  toPacket() {
    const _ = 4 + this.data.length, A = Buffer.allocUnsafe(_), R = new IN(0, A, 0, _);
    return R.offset = 4, R.writeBuffer(this.data), R;
  }
  static fromPacket(_) {
    const A = _.readBuffer();
    return new WA(A);
...
var NN = WA;
const HA = UE(), $ = K, _E = new Array(256);
class yA {
  constructor(_) {
    this.columns = _ || [];
  }
  static toPacket(_, A) {
    ...
    _.forEach((e) => {
      ...
      n += $.lengthCodedStringLength(e.toString(10), A);
    }), n = n + 2;
    const I = Buffer.allocUnsafe(n + 4), N = new $(0, I, 0, n + 4);
    N.offset = 4, N.writeInt8(0);
    let t = 0, i = 1;
    return _.forEach((e) => {
      e.type === HA.NULL && (t += i), i *= 2, i === 256 && (N.writeInt8(t), t = 0, i = 1);
    }), i !== 1 && N.writeInt8(t), _.forEach((e) => {
      if (e === null) {
        N.writeNull();
        return;
      }
      ...
      N.writeLengthCodedString(e.toString(10), A);
    }), N;
  }
  ...
  static fromPacket(_, A) {
    ...
    A.readInt8();
    const n = Math.floor((_.length + 7 + 2) / 8);
    A.skip(n);
    for (let I = 0; I < R.length; ++I)
      R[I] = _E[_[I].columnType].apply(A);
    return new yA(R);
  }
}
_E[HA.DECIMAL] = $.prototype.readLengthCodedString;
_E[1] = $.prototype.readInt8;
_E[2] = $.prototype.readInt16;
_E[3] = $.prototype.readInt32;
_E[4] = $.prototype.readFloat;
_E[5] = $.prototype.readDouble;
_E[6] = $.prototype.assertInvalid;
_E[7] = $.prototype.readTimestamp;
_E[8] = $.prototype.readInt64;
_E[9] = $.prototype.readInt32;
_E[10] = $.prototype.readTimestamp;
_E[11] = $.prototype.readTime;
_E[12] = $.prototype.readDateTime;
_E[13] = $.prototype.readInt16;
_E[HA.VAR_STRING] = $.prototype.readLengthCodedString;
var TN = yA, uE = {
  QUIT: 1,
  INIT_DB: 2,
  QUERY: 3,
...
let iN = class {
  ...
  toPacket() {
    const _ = 15 + Buffer.byteLength(this.filename, "utf8"), A = Buffer.allocUnsafe(_), R = new eN(0, A, 0, _);
    return R.offset = 4, R.writeInt8(tN.BINLOG_DUMP), R.writeInt32(this.binlogPos), R.writeInt16(this.flags), R.writeInt32(this.serverId), R.writeString(this.filename), R;
  }
};
var aN = iN, G = {};
G.LONG_PASSWORD = 1;
...
G.INTERACTIVE = 1024;
G.SSL = 2048;
G.IGNORE_SIGPIPE = 4096;
G.TRANSACTIONS = 8192;
G.RESERVED = 16384;
G.SECURE_CONNECTION = 32768;
G.MULTI_STATEMENTS = 65536;
G.MULTI_RESULTS = 131072;
G.PS_MULTI_RESULTS = 262144;
G.PLUGIN_AUTH = 524288;
G.CONNECT_ATTRS = 1048576;
G.PLUGIN_AUTH_LENENC_CLIENT_DATA = 2097152;
G.CAN_HANDLE_EXPIRED_PASSWORDS = 4194304;
G.SESSION_TRACK = 8388608;
G.DEPRECATE_EOF = 16777216;
G.SSL_VERIFY_SERVER_CERT = 1073741824;
G.REMEMBER_OPTIONS = 2147483648;
G.MULTI_FACTOR_AUTHENTICATION = 268435456;
var YE = {};
(function(E) {
  const _ = r_;
  function A(N, t, i) {
    const e = _.createHash("sha1");
    return e.update(N), t && e.update(t), i && e.update(i), e.digest();
  }
  function R(N, t) {
  ...
  function n(N, t, i) {
    ...
    const e = A(N);
    return E.calculateTokenFromPasswordSha(e, t, i);
  }
  E.calculateTokenFromPasswordSha = function(N, t, i) {
    const e = t.slice(0, 8), T = i.slice(0, 12), a = A(N), u = A(e, T, a);
    return R(u, N);
  }, E.calculateToken = n, E.verifyToken = function(N, t, i, e) {
    const T = R(i, A(N, t, e));
    return A(T).compare(e) === 0;
  }, E.doubleSha1 = function(N) {
    return A(A(N));
  };
  function I(N, t) {
    const i = Buffer.allocUnsafe(N.length), e = t.length;
...
const ON = uE, CN = G, O_ = K, OR = YE, DN = SE();
let uN = class {
  constructor(_) {
    ...
    let A;
    this.passwordSha1 ? A = OR.calculateTokenFromPasswordSha(
      this.passwordSha1,
      ...
      this.authPluginData1,
      this.authPluginData2
    ), this.authToken = A, this.charsetNumber = _.charsetNumber;
  }
  // TODO
  // ChangeUser.fromPacket = function(packet)
  // };
  serializeToBuffer(_) {
    const A = (I) => this.flags & CN[I], R = new O_(0, _, 0, _.length);
    R.offset = 4;
    const n = DN[this.charsetNumber];
    if (R.writeInt8(ON.CHANGE_USER), R.writeNullTerminatedString(this.user, n), A("SECURE_CONNECTION") ? (R.writeInt8(this.authToken.length), R.writeBuffer(this.authToken)) : (R.writeBuffer(this.authToken), R.writeInt8(0)), R.writeNullTerminatedString(this.database, n), R.writeInt16(this.charsetNumber), A("PLUGIN_AUTH") && R.writeNullTerminatedString("mysql_native_password", "latin1"), A("CONNECT_ATTRS")) {
      const I = this.connectAttributes, N = Object.keys(I);
      let t = 0;
      for (let i = 0; i < N.length; ++i)
        t += O_.lengthCodedStringLength(N[i], n), t += O_.lengthCodedStringLength(
          I[N[i]],
          n
        );
      R.writeLengthCodedNumber(t);
      for (let i = 0; i < N.length; ++i)
        R.writeLengthCodedString(N[i], n), R.writeLengthCodedString(
          I[N[i]],
          n
        );
    }
  ...
  toPacket() {
    ...
    const _ = this.serializeToBuffer(O_.MockBuffer());
...
let MN = class {
  ...
  toPacket() {
    const _ = new cN(0, Buffer.allocUnsafe(9), 0, 9);
    return _.offset = 4, _.writeInt8(BN.STMT_CLOSE), _.writeInt32(this.id), _;
  }
};
var LN = MN, q = {};
q.NOT_NULL = 1;
...
q.BINARY = 128;
q.ENUM = 256;
q.AUTO_INCREMENT = 512;
q.TIMESTAMP = 1024;
q.SET = 2048;
q.NO_DEFAULT_VALUE = 4096;
q.ON_UPDATE_NOW = 8192;
q.NUM = 32768;
const CR = K, hn = zE, J_ = SE(), DR = ["catalog", "schema", "table", "orgTable", "name", "orgName"];
class Un {
  constructor(_, A) {
    this._buf = _.buffer, this._clientEncoding = A, this._catalogLength = _.readLengthCodedNumber(), this._catalogStart = _.offset, _.offset += this._catalogLength, this._schemaLength = _.readLengthCodedNumber(), this._schemaStart = _.offset, _.offset += this._schemaLength, this._tableLength = _.readLengthCodedNumber(), this._tableStart = _.offset, _.offset += this._tableLength, this._orgTableLength = _.readLengthCodedNumber(), this._orgTableStart = _.offset, _.offset += this._orgTableLength;
    const R = _.readLengthCodedNumber(), n = _.offset;
    _.offset += R, this._orgNameLength = _.readLengthCodedNumber(), this._orgNameStart = _.offset, _.offset += this._orgNameLength, _.skip(1), this.characterSet = _.readInt16(), this.encoding = J_[this.characterSet], this.name = hn.decode(
      this._buf,
  ...
  [Symbol.for("nodejs.util.inspect.custom")](_, A, R) {
    const n = UE(), I = [];
    for (const a in n)
      I[n[a]] = a;
    const N = q, t = [], i = this.flags;
    for (const a in N)
      i & N[a] && (a === "PRI_KEY" ? t.push("PRIMARY KEY") : a === "NOT_NULL" ? t.push("NOT NULL") : a === "BINARY" || a === "MULTIPLE_KEY" || a === "NO_DEFAULT_VALUE" || a === "BLOB" || a === "UNSIGNED" || a === "TIMESTAMP" || (a === "ON_UPDATE_NOW" ? t.push("ON UPDATE CURRENT_TIMESTAMP") : t.push(a)));
    if (_ > 1)
      return R({
        ...this.inspect(),
    ...
    const e = this.flags & N.UNSIGNED;
    let T = I[this.columnType];
    return T === "BLOB" ? this.columnLength === 4294967295 ? T = "LONGTEXT" : this.columnLength === 67108860 ? T = "MEDIUMTEXT" : this.columnLength === 262140 ? T = "TEXT" : this.columnLength === 1020 ? T = "TINYTEXT" : T = `BLOB(${this.columnLength})` : T === "VAR_STRING" ? T = `VARCHAR(${Math.ceil(this.columnLength / 4)})` : T === "TINY" ? this.columnLength === 3 && e || this.columnLength === 4 && !e ? T = "TINYINT" : T = `TINYINT(${this.columnLength})` : T === "LONGLONG" ? this.columnLength === 20 ? T = "BIGINT" : T = `BIGINT(${this.columnLength})` : T === "SHORT" ? e && this.columnLength === 5 || !e && this.columnLength === 6 ? T = "SMALLINT" : T = `SMALLINT(${this.columnLength})` : T === "LONG" ? e && this.columnLength === 10 || !e && this.columnLength === 11 ? T = "INT" : T = `INT(${this.columnLength})` : T === "INT24" ? e && this.columnLength === 8 || !e && this.columnLength === 9 ? T = "MEDIUMINT" : T = `MEDIUMINT(${this.columnLength})` : T === "DOUBLE" ? this.columnLength === 22 && this.decimals === 31 ? T = "DOUBLE" : T = `DOUBLE(${this.columnLength},${this.decimals})` : T === "FLOAT" ? this.columnLength === 12 && this.decimals === 31 ? T = "FLOAT" : T = `FLOAT(${this.columnLength},${this.decimals})` : T === "NEWDECIMAL" ? this.columnLength === 11 && this.decimals === 0 ? T = "DECIMAL" : this.decimals === 0 ? e ? T = `DECIMAL(${this.columnLength})` : T = `DECIMAL(${this.columnLength - 1})` : T = `DECIMAL(${this.columnLength - 2},${this.decimals})` : T = `${I[this.columnType]}(${this.columnLength})`, e && (T += " UNSIGNED"), `\`${this.name}\` ${[T, ...t].join(" ")}`;
  }
  static toPacket(_, A) {
    let R = 17;
    DR.forEach((t) => {
      R += CR.lengthCodedStringLength(
        _[t],
        J_[_.characterSet]
      );
    });
    const n = Buffer.allocUnsafe(R), I = new CR(A, n, 0, R);
    function N(t) {
    ...
    return I.offset = 4, DR.forEach(N), I.writeInt8(12), I.writeInt16(_.characterSet), I.writeInt32(_.columnLength), I.writeInt8(_.columnType), I.writeInt16(_.flags), I.writeInt8(_.decimals), I.writeInt16(0), I;
  }
...
const n_ = function(E) {
  Object.defineProperty(Un.prototype, E, {
    get: function() {
      const _ = this[`_${E}Start`], A = _ + this[`_${E}Length`], R = hn.decode(
        this._buf,
        this.encoding === "binary" ? this._clientEncoding : this.encoding,
        _,
        A
...
const gN = UN, lN = uE, m = UE(), rE = K, sN = SE();
function fN(E) {
  return Array.isArray(E) || E.constructor === Object || typeof E.toJSON == "function" && !Buffer.isBuffer(E);
}
function rN(E, _, A) {
  let R = m.VAR_STRING, n, I = function(N) {
    return rE.prototype.writeLengthCodedString.call(this, N, _);
  };
  if (E !== null)
    switch (typeof E) {
      case "undefined":
      ...
      case "number":
        R = m.DOUBLE, n = 8, I = rE.prototype.writeDouble;
        break;
      case "boolean":
        E = E | 0, R = m.TINY, n = 1, I = rE.prototype.writeInt8;
        break;
      case "object":
        Object.prototype.toString.call(E) === "[object Date]" ? (R = m.DATETIME, n = 12, I = function(N) {
          return rE.prototype.writeDate.call(this, N, A);
        }) : fN(E) ? (E = JSON.stringify(E), R = m.JSON) : Buffer.isBuffer(E) && (n = rE.lengthCodedNumberLength(E.length) + E.length, I = rE.prototype.writeLengthCodedBuffer);
        break;
      default:
        E = E.toString();
    }
    ...
    E = "", R = m.NULL;
  return n || (n = rE.lengthCodedStringLength(E, _)), { value: E, type: R, length: n, writer: I };
}
let dN = class {
  constructor(_, A, R, n) {
    this.id = _, this.parameters = A, this.encoding = sN[R], this.timezone = n;
  }
  static fromPacket(_, A) {
    const R = _.readInt32(), n = _.readInt8(), I = _.readInt32();
    let N = _.offset;
    for (; N < _.end - 1 && !((_.buffer[N + 1] === m.VAR_STRING || _.buffer[N + 1] === m.NULL || _.buffer[N + 1] === m.DOUBLE || _.buffer[N + 1] === m.TINY || _.buffer[N + 1] === m.DATETIME || _.buffer[N + 1] === m.JSON) && _.buffer[N] === 1 && _.buffer[N + 2] === 0); )
      _.readInt8(), N++;
    const t = [];
    for (let e = _.offset + 1; e < _.end - 1; e++)
      (_.buffer[e] === m.VAR_STRING || _.buffer[e] === m.NULL || _.buffer[e] === m.DOUBLE || _.buffer[e] === m.TINY || _.buffer[e] === m.DATETIME || _.buffer[e] === m.JSON) && _.buffer[e + 1] === 0 && (t.push(_.buffer[e]), _.skip(2));
    _.skip(1);
    const i = [];
    for (let e = 0; e < t.length; e++)
      t[e] === m.VAR_STRING ? i.push(_.readLengthCodedString(A)) : t[e] === m.DOUBLE ? i.push(_.readDouble()) : t[e] === m.TINY ? i.push(_.readInt8()) : t[e] === m.DATETIME ? i.push(_.readDateTime()) : t[e] === m.JSON && i.push(JSON.parse(_.readLengthCodedString(A))), t[e] === m.NULL && i.push(null);
    return { stmtId: R, flags: n, iterationCount: I, values: i };
  }
  toPacket() {
    let _ = 14, A;
    this.parameters && this.parameters.length > 0 && (_ += Math.floor((this.parameters.length + 7) / 8), _ += 1, _ += 2 * this.parameters.length, A = this.parameters.map(
      (I) => rN(I, this.encoding, this.timezone)
    ), _ += A.reduce(
      (I, N) => I + N.length,
      0
    ));
    const R = Buffer.allocUnsafe(_), n = new rE(0, R, 0, _);
    if (n.offset = 4, n.writeInt8(lN.STMT_EXECUTE), n.writeInt32(this.id), n.writeInt8(gN.NO_CURSOR), n.writeInt32(1), A) {
      let I = 0, N = 1;
      A.forEach((t) => {
        t.type === m.NULL && (I += N), N *= 2, N === 256 && (n.writeInt8(I), I = 0, N = 1);
      }), N !== 1 && n.writeInt8(I), n.writeInt8(1), A.forEach((t) => {
        n.writeInt8(t.type), n.writeInt8(0);
      }), A.forEach((t) => {
        t.type !== m.NULL && t.writer.call(n, t.value);
      });
    }
    return n;
  }
...
class vA {
  ...
  setScrambleData(_) {
    r_.randomBytes(20, (A, R) => {
      if (A) {
        _(A);
        return;
      }
      this.authPluginData1 = R.slice(0, 8), this.authPluginData2 = R.slice(8, 20), _();
    });
  }
  toPacket(_) {
    const A = 68 + Buffer.byteLength(this.serverVersion, "utf8"), R = Buffer.alloc(A + 4, 0), n = new wN(_, R, 0, A + 4);
  ...
  static fromPacket(_) {
    const A = {};
    A.protocolVersion = _.readInt8(), A.serverVersion = _.readNullTerminatedString("cesu8"), A.connectionId = _.readInt32(), A.authPluginData1 = _.readBuffer(8), _.skip(1);
    const R = Buffer.allocUnsafe(4);
    if (R[0] = _.readInt8(), R[1] = _.readInt8(), _.haveMoreData() ? (A.characterSet = _.readInt8(), A.statusFlags = _.readInt16(), R[2] = _.readInt8(), R[3] = _.readInt8(), A.capabilityFlags = R.readUInt32LE(0), A.capabilityFlags & z_.PLUGIN_AUTH ? A.authPluginDataLength = _.readInt8() : (A.authPluginDataLength = 0, _.skip(1)), _.skip(10)) : A.capabilityFlags = R.readUInt16LE(0), A.capabilityFlags & z_.SECURE_CONNECTION) {
      const I = A.authPluginDataLength;
      if (I === 0)
        A.authPluginDataLength = 20, A.authPluginData2 = _.readBuffer(12), _.skip(1);
      else {
        const N = Math.max(13, I - 8);
        A.authPluginData2 = _.readBuffer(N);
      }
    }
    return A.capabilityFlags & z_.PLUGIN_AUTH && (A.authPluginName = _.readNullTerminatedString("ascii")), new vA(A);
  }
}
var FN = vA;
const uR = G, SR = SE(), C_ = K, cR = YE;
class QN {
  constructor(_) {
    ...
    let A;
    this.passwordSha1 ? A = cR.calculateTokenFromPasswordSha(
      this.passwordSha1,
      this.authPluginData1,
      this.authPluginData2
    ) : A = cR.calculateToken(
      ...
      this.authPluginData1,
      this.authPluginData2
    ), this.authToken = A, this.charsetNumber = _.charsetNumber, this.encoding = SR[_.charsetNumber], this.connectAttributes = _.connectAttributes;
  }
  serializeResponse(_) {
    const A = (N) => this.clientFlags & uR[N], R = new C_(0, _, 0, _.length);
    R.offset = 4, R.writeInt32(this.clientFlags), R.writeInt32(0), R.writeInt8(this.charsetNumber), R.skip(23);
    const n = this.encoding;
    R.writeNullTerminatedString(this.user, n);
    let I;
    if (A("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (R.writeLengthCodedNumber(this.authToken.length), R.writeBuffer(this.authToken)) : A("SECURE_CONNECTION") ? (R.writeInt8(this.authToken.length), R.writeBuffer(this.authToken)) : (R.writeBuffer(this.authToken), R.writeInt8(0)), A("CONNECT_WITH_DB") && R.writeNullTerminatedString(this.database, n), A("PLUGIN_AUTH") && R.writeNullTerminatedString("mysql_native_password", "latin1"), A("CONNECT_ATTRS")) {
      const N = this.connectAttributes || {}, t = Object.keys(N);
      let i = 0;
      for (I = 0; I < t.length; ++I)
        i += C_.lengthCodedStringLength(t[I], n), i += C_.lengthCodedStringLength(
          N[t[I]],
          n
        );
      for (R.writeLengthCodedNumber(i), I = 0; I < t.length; ++I)
        R.writeLengthCodedString(t[I], n), R.writeLengthCodedString(
          N[t[I]],
          n
        );
    }
  ...
  toPacket() {
    ...
    if (typeof this.database != "string")
      throw new Error('"database" connection config property must be a string');
    const _ = this.serializeResponse(C_.MockBuffer());
    return this.serializeResponse(Buffer.alloc(_.offset));
  }
  static fromPacket(_) {
    const A = {};
    A.clientFlags = _.readInt32();
    function R(N) {
      return A.clientFlags & uR[N];
    }
    A.maxPacketSize = _.readInt32(), A.charsetNumber = _.readInt8();
    const n = SR[A.charsetNumber];
    A.encoding = n, _.skip(23), A.user = _.readNullTerminatedString(n);
    let I;
    if (R("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (I = _.readLengthCodedNumber(n), A.authToken = _.readBuffer(I)) : R("SECURE_CONNECTION") ? (I = _.readInt8(), A.authToken = _.readBuffer(I)) : A.authToken = _.readNullTerminatedString(n), R("CONNECT_WITH_DB") && (A.database = _.readNullTerminatedString(n)), R("PLUGIN_AUTH") && (A.authPluginName = _.readNullTerminatedString(n)), R("CONNECT_ATTRS")) {
      const N = _.readLengthCodedNumber(n), t = _.offset + N, i = {};
      for (; _.offset < t; )
        i[_.readLengthCodedString(n)] = _.readLengthCodedString(n);
      A.connectAttributes = i;
    }
    return A;
  }
}
var PN = QN;
const VN = K, bN = uE, oN = zE, YN = SE();
class mN {
  constructor(_, A) {
    this.query = _, this.charsetNumber = A, this.encoding = YN[A];
  }
  toPacket() {
    const _ = oN.encode(this.query, this.encoding), A = 5 + _.length, R = Buffer.allocUnsafe(A), n = new VN(0, R, 0, A);
    return n.offset = 4, n.writeInt8(bN.STMT_PREPARE), n.writeBuffer(_), n;
  }
}
var WN = mN;
class HN {
...
var yN = HN;
const vN = K, XN = uE, KN = zE, jN = SE();
let JN = class {
  constructor(_, A) {
    this.query = _, this.charsetNumber = A, this.encoding = jN[A];
  }
  toPacket() {
    const _ = KN.encode(this.query, this.encoding), A = 5 + _.length, R = Buffer.allocUnsafe(A), n = new vN(0, R, 0, A);
    return n.offset = 4, n.writeInt8(XN.QUERY), n.writeBuffer(_), n;
  }
};
var zN = JN;
const ZN = K, kN = uE;
let qN = class {
  ...
  toPacket() {
    ...
    Buffer.byteLength(this.slaveHostname, "utf8") + Buffer.byteLength(this.slaveUser, "utf8") + Buffer.byteLength(this.slavePassword, "utf8") + 3 + 4, A = Buffer.allocUnsafe(_), R = new ZN(0, A, 0, _);
    return R.offset = 4, R.writeInt8(kN.REGISTER_SLAVE), R.writeInt32(this.serverId), R.writeInt8(Buffer.byteLength(this.slaveHostname, "utf8")), R.writeString(this.slaveHostname), R.writeInt8(Buffer.byteLength(this.slaveUser, "utf8")), R.writeString(this.slaveUser), R.writeInt8(Buffer.byteLength(this.slavePassword, "utf8")), R.writeString(this.slavePassword), R.writeInt16(this.slavePort), R.writeInt32(this.replicationRank), R.writeInt32(this.masterId), R;
  }
};
var pN = qN, AE = {};
AE.SERVER_STATUS_IN_TRANS = 1;
AE.SERVER_STATUS_AUTOCOMMIT = 2;
AE.SERVER_MORE_RESULTS_EXISTS = 8;
AE.SERVER_QUERY_NO_GOOD_INDEX_USED = 16;
AE.SERVER_QUERY_NO_INDEX_USED = 32;
AE.SERVER_STATUS_CURSOR_EXISTS = 64;
AE.SERVER_STATUS_LAST_ROW_SENT = 128;
AE.SERVER_STATUS_DB_DROPPED = 256;
AE.SERVER_STATUS_NO_BACKSLASH_ESCAPES = 512;
AE.SERVER_STATUS_METADATA_CHANGED = 1024;
AE.SERVER_QUERY_WAS_SLOW = 2048;
AE.SERVER_PS_OUT_PARAMS = 4096;
AE.SERVER_STATUS_IN_TRANS_READONLY = 8192;
AE.SERVER_SESSION_STATE_CHANGED = 16384;
var xN = {
  big5: 1,
  latin2: 2,
  dec8: 3,
...
const Z_ = K, $N = G, ET = AE, _T = xN, D_ = gn;
class AT {
  constructor(_, A) {
    const R = A.config.bigNumberStrings, n = A.serverEncoding, I = A._handshakePacket.capabilityFlags, N = function(e) {
      return I & $N[e];
    };
    if (_.buffer[_.offset] !== 0) {
      this.fieldCount = _.readLengthCodedNumber(), this.fieldCount === null && (this.infileName = _.readString(void 0, n));
      return;
    }
    this.fieldCount = _.readInt8(), this.affectedRows = _.readLengthCodedNumber(R), this.insertId = _.readLengthCodedNumberSigned(R), this.info = "", N("PROTOCOL_41") ? (this.serverStatus = _.readInt16(), this.warningStatus = _.readInt16()) : N("TRANSACTIONS") && (this.serverStatus = _.readInt16());
    let t = null;
    if (N("SESSION_TRACK") && _.offset < _.end) {
      if (this.info = _.readLengthCodedString(n), this.serverStatus && ET.SERVER_SESSION_STATE_CHANGED) {
        let e = _.offset < _.end ? _.readLengthCodedNumber() : 0;
        const T = _.offset + e;
        let a, u, S;
        for (e > 0 && (t = {
          systemVariables: {},
          ...
          if (a = _.readInt8(), e = _.readLengthCodedNumber(), S = _.offset + e, a === D_.SYSTEM_VARIABLES) {
            u = _.readLengthCodedString(n);
            const O = _.readLengthCodedString(n);
            if (t.systemVariables[u] = O, u === "character_set_client") {
              const D = _T[O];
              typeof D < "u" && (A.config.charsetNumber = D);
            }
          } else if (a === D_.SCHEMA)
            u = _.readLengthCodedString(n), t.schema = u;
          else if (a === D_.STATE_CHANGE)
            t.trackStateChange = _.readLengthCodedString(n);
          else if (a === D_.STATE_GTIDS) {
      ...
      this.info = _.readString(void 0, n);
    t && (this.stateChanges = t);
    const i = this.info.match(/\schanged:\s*(\d+)/i);
    i !== null ? this.changedRows = parseInt(i[1], 10) : this.changedRows = 0;
  }
  // TODO: should be consistent instance member, but it's just easier here to have just function
  static toPacket(_, A) {
    let R = 4 + Z_.lengthCodedNumberLength(_);
    typeof A < "u" && (R += Z_.lengthCodedNumberLength(A));
    const n = Buffer.allocUnsafe(R), I = new Z_(0, n, 0, R);
    return I.offset = 4, I.writeLengthCodedNumber(_), typeof A < "u" && I.writeLengthCodedNumber(A), I;
  }
}
var RT = AT;
const nT = G, IT = K;
class NT {
  constructor(_, A) {
    this.clientFlags = _ | nT.SSL, this.charset = A;
  }
  toPacket() {
    const A = Buffer.allocUnsafe(36), R = new IT(0, A, 0, 36);
    return A.fill(0), R.offset = 4, R.writeInt32(this.clientFlags), R.writeInt32(0), R.writeInt8(this.charset), R;
  }
}
var TT = NT;
const BR = K;
class XA {
  ...
  static toPacket(_, A) {
    ...
    _.forEach((t) => {
      ...
      n += BR.lengthCodedStringLength(t.toString(10), A);
    });
    const I = Buffer.allocUnsafe(n + 4), N = new BR(0, I, 0, n + 4);
    return N.offset = 4, _.forEach((t) => {
      if (t === null) {
        N.writeNull();
        return;
      }
      ...
      N.writeLengthCodedString(t.toString(10), A);
    }), N;
  }
}
var eT = XA;
(function(E, _) {
  ...
  Object.entries({
    AuthNextFactor: EN,
    AuthSwitchRequest: AN,
    AuthSwitchRequestMoreData: nN,
    AuthSwitchResponse: NN,
    BinaryRow: TN,
    BinlogDump: aN,
    ChangeUser: SN,
    CloseStatement: LN,
    ...
    Execute: GN,
    Handshake: FN,
    HandshakeResponse: PN,
    PrepareStatement: WN,
    PreparedStatementHeader: yN,
    Query: zN,
    RegisterSlave: pN,
    ResultSetHeader: RT,
    SSLRequest: TT,
    ...
    if (E.exports[OE] = r, A.env.NODE_DEBUG && r.prototype.toPacket) {
      const g = r.prototype.toPacket;
  ...
  class o {
    static toPacket(r, g) {
      r = r || {};
      const U = r.affectedRows || 0, C = r.insertId || 0, f = r.serverStatus || 0, w = r.warningCount || 0, Y = r.message || "";
      let P = 9 + d.lengthCodedNumberLength(U);
      P += d.lengthCodedNumberLength(C);
      const j = Buffer.allocUnsafe(P), W = new d(0, j, 0, P);
      return W.offset = 4, W.writeInt8(0), W.writeLengthCodedNumber(U), W.writeLengthCodedNumber(C), W.writeInt16(f), W.writeInt16(w), W.writeString(Y, g), W._name = "OK", W;
    }
  }
  _.OK = o;
  class Z {
    static toPacket(r, g) {
      typeof r > "u" && (r = 0), typeof g > "u" && (g = 0);
      const U = new d(0, Buffer.allocUnsafe(9), 0, 9);
      return U.offset = 4, U.writeInt8(254), U.writeInt16(r), U.writeInt16(g), U._name = "EOF", U;
    }
  }
  _.EOF = Z;
  class RE {
    static toPacket(r, g) {
      const U = 13 + Buffer.byteLength(r.message, "utf8"), C = new d(0, Buffer.allocUnsafe(U), 0, U);
      return C.offset = 4, C.writeInt8(255), C.writeInt16(r.code), C.writeString("#_____", g), C.writeString(r.message, g), C._name = "Error", C;
    }
    static fromPacket(r) {
      r.readInt8();
      const g = r.readInt16();
      r.readString(1, "ascii"), r.readString(5, "ascii");
      const U = r.readNullTerminatedString("utf8"), C = new RE();
      return C.message = U, C.code = g, C;
    }
  }
  _.Error = RE;
})(LA, LA.exports);
var CE = LA.exports;
const tT = GE.EventEmitter, iT = dA;
let aT = class extends tT {
  ...
  execute(_, A) {
    if (this.next || (this.next = this.start, A._resetSequenceId()), _ && _.isError()) {
      const R = _.asError(A.clientEncoding);
      return R.sql = this.sql || this.query, this.queryTimeout && (iT.clearTimeout(this.queryTimeout), this.queryTimeout = null), this.onResult ? (this.onResult(R), this.emit("end")) : (this.emit("error", R), this.emit("end")), !0;
    }
    return this.next = this.next(_, A), this.next ? !1 : (this.emit("end"), !0);
  }
};
var DE = aT, k_, MR;
function OT() {
  if (MR) return k_;
  MR = 1;
  const E = "sha256_password", _ = r_, { xorRotating: A } = YE, R = tn, n = Buffer.from([1]), I = 0, N = 1, t = -1;
  function i(e, T, a) {
    const u = A(Buffer.from(`${e}\0`, "utf8"), T);
    return _.publicEncrypt(a, u);
  }
  return k_ = (e = {}) => ({ connection: T }) => {
    let a = 0, u = null;
    const S = T.config.password, O = (D) => {
      const c = i(S, u, D);
      return a = t, c;
    };
    return (D) => {
      switch (a) {
        case I:
          return T.stream instanceof R.TLSSocket && T.stream.encrypted === !0 ? Buffer.from(`${S}\0`, "utf8") : (u = D.slice(0, 20), e.serverPublicKey ? O(e.serverPublicKey) : (a = N, n));
        case N:
          return e.onServerPublicKey && e.onServerPublicKey(D), O(D);
        case t:
          throw new Error(
...
function CT() {
  if (LR) return q_;
  LR = 1;
  const E = "caching_sha2_password", _ = r_, { xor: A, xorRotating: R } = YE, n = Buffer.from([2]), I = Buffer.from([3]), N = Buffer.from([4]), t = 0, i = 1, e = 2, T = -1;
  function a(O) {
    const D = _.createHash("sha256");
    return D.update(O), D.digest();
  }
  function u(O, D) {
    if (!O)
      return Buffer.alloc(0);
    const c = a(Buffer.from(O)), h = a(c), M = a(Buffer.concat([h, D]));
    return A(c, M);
  }
  function S(O, D, c) {
    const h = R(Buffer.from(`${O}\0`, "utf8"), D);
  ...
  return q_ = (O = {}) => ({ connection: D }) => {
    let c = 0, h = null;
    const M = D.config.password, L = (B) => {
      const s = S(M, h, B);
      return c = T, s;
    };
    return (B) => {
      switch (c) {
        case t:
          return h = B.slice(0, 20), c = i, u(M, h);
        case i:
          if (I.equals(B))
            return c = T, null;
          if (N.equals(B))
            return (typeof O.overrideIsSecure > "u" ? D.config.ssl || D.config.socketPath : O.overrideIsSecure) ? (c = T, Buffer.from(`${M}\0`, "utf8")) : O.serverPublicKey ? L(O.serverPublicKey) : (c = e, n);
          throw new Error(
            `Invalid AuthMoreData packet received by ${E} plugin in STATE_TOKEN_SENT state.`
          );
        case e:
          return O.onServerPublicKey && O.onServerPublicKey(B), L(B);
        case T:
          throw new Error(
            `Unexpected data in AuthMoreData packet received by ${E} plugin in STATE_FINAL state.`
          );
...
function DT() {
  ...
  hR = 1;
  const E = YE;
  return p_ = (_) => ({ connection: A, command: R }) => {
    const n = R.password || _.password || A.config.password, I = R.passwordSha1 || _.passwordSha1 || A.config.passwordSha1;
    return (N) => {
      const t = N.slice(0, 8), i = N.slice(8, 20);
      let e;
      return I ? e = E.calculateTokenFromPasswordSha(
        I,
        t,
        i
      ) : e = E.calculateToken(
...
function uT() {
  ...
  UR = 1;
  function E(A) {
    return Buffer.from(`${A}\0`);
  }
  return x_ = (A) => function({ connection: n, command: I }) {
    const N = I.password || A.password || n.config.password;
    return function() {
      return E(N);
    };
  }, x_;
...
function ST() {
  if (gR) return $_;
  gR = 1;
  const E = CE, _ = OT(), A = CT(), R = DT(), n = uT(), I = {
    sha256_password: _({}),
    caching_sha2_password: A({}),
    mysql_native_password: R({}),
    mysql_clear_password: n({})
  };
  function N() {
    console.warn(
  ...
  function i(T, a, u) {
    const { pluginName: S, pluginData: O } = E.AuthSwitchRequest.fromPacket(T);
    let D = a.config.authPlugins && a.config.authPlugins[S];
    if (a.config.authSwitchHandler && S !== "mysql_native_password") {
      const c = a.config.authSwitchHandler;
      N(), c({ pluginName: S, pluginData: O }, (h, M) => {
        if (h)
          return t(h, u);
        a.writePacket(new E.AuthSwitchResponse(M).toPacket());
      });
      return;
    }
    if (D || (D = I[S]), !D)
    ...
    a._authPlugin = D({ connection: a, command: u }), Promise.resolve(a._authPlugin(O)).then((c) => {
  ...
  function e(T, a, u) {
    const { data: S } = E.AuthSwitchRequestMoreData.fromPacket(T);
    if (a.config.authSwitchHandler) {
      const O = a.config.authSwitchHandler;
      N(), O({ pluginData: S }, (D, c) => {
        if (D)
          return t(D, u);
    ...
    Promise.resolve(a._authPlugin(S)).then((O) => {
      O && a.writePacket(new E.AuthSwitchResponse(O).toPacket());
    }).catch((O) => {
      t(O, u);
    });
  }
  return $_ = {
    authSwitchRequest: i,
    authSwitchRequestMoreData: e
  }, $_;
}
var EA = { exports: {} }, lR;
function cT() {
  return lR || (lR = 1, function(E) {
    var _ = GE.EventEmitter, A = Cn, R = 3e3, n = 0, I = "closed", N = "drained", t = function(e) {
      _.call(this), e && e > 0 ? this.timeout = e : this.timeout = R, this.status = i.STATUS_IDLE, this.curId = n, this.queue = [];
    };
    A.inherits(t, _), t.prototype.push = function(e, T, a) {
      if (this.status !== i.STATUS_IDLE && this.status !== i.STATUS_BUSY)
        return !1;
      if (typeof e != "function")
        throw new Error("fn should be a function.");
      if (this.queue.push({ fn: e, ontimeout: T, timeout: a }), this.status === i.STATUS_IDLE) {
      ...
      this.status !== i.STATUS_IDLE && this.status !== i.STATUS_BUSY || (e ? (this.status = i.STATUS_DRAINED, this.timerId && (clearTimeout(this.timerId), this.timerId = void 0), this.emit(N)) : (this.status = i.STATUS_CLOSED, this.emit(I)));
    }, t.prototype._next = function(e) {
      if (!(e !== this.curId || this.status !== i.STATUS_BUSY && this.status !== i.STATUS_CLOSED)) {
        ...
        var a = this;
        T.id = ++this.curId;
        var u = T.timeout > 0 ? T.timeout : this.timeout;
        u = u > 0 ? u : R, this.timerId = setTimeout(function() {
          process.nextTick(function() {
            a._next(T.id);
          }), a.emit("timeout", T), T.ontimeout && T.ontimeout();
        }, u);
        try {
          T.fn({
            done: function() {
          ...
          a.emit("error", S, T), process.nextTick(function() {
            a._next(T.id);
          });
        }
      }
    ...
    var i = E.exports;
    i.STATUS_IDLE = 0, i.STATUS_BUSY = 1, i.STATUS_CLOSED = 2, i.STATUS_DRAINED = 3, i.createQueue = function(e) {
      return new t(e);
    };
  }(EA)), EA.exports;
}
...
function MT() {
  ...
  const E = iI, _ = Ln;
  function A(I) {
    const N = this, t = I.readInt24(), i = I.readBuffer();
    t !== 0 ? N.inflateQueue.push((e) => {
      E.inflate(i, (T, a) => {
        if (T) {
          N._handleNetworkError(T);
          return;
        }
        N._bumpCompressedSequenceId(I.numPackets), N._inflatedPacketsParser.execute(a), e.done();
      });
    }) : N.inflateQueue.push((e) => {
      N._bumpCompressedSequenceId(I.numPackets), N._inflatedPacketsParser.execute(i), e.done();
    });
  }
  function R(I) {
    let t;
    if (I.length > 16777210) {
      for (t = 0; t < I.length; t += 16777210)
        R.call(
          // eslint-disable-next-line no-invalid-this
          this,
          I.slice(t, t + 16777210)
        );
      return;
    }
    const i = this;
    let e = I.length;
    const T = Buffer.allocUnsafe(7);
    (function(a) {
      i.deflateQueue.push((u) => {
        E.deflate(I, (S, O) => {
          ...
          let D = O.length;
          D < e ? (T.writeUInt8(D & 255, 0), T.writeUInt16LE(D >> 8, 1), T.writeUInt8(a, 3), T.writeUInt8(e & 255, 4), T.writeUInt16LE(e >> 8, 5), i.writeUncompressed(T), i.writeUncompressed(O)) : (D = e, e = 0, T.writeUInt8(D & 255, 0), T.writeUInt16LE(D >> 8, 1), T.writeUInt8(a, 3), T.writeUInt8(e & 255, 4), T.writeUInt16LE(e >> 8, 5), i.writeUncompressed(T), i.writeUncompressed(I)), u.done();
        });
      });
    })(i.compressedSequenceId), i._bumpCompressedSequenceId(1);
  }
  function n(I) {
    I._lastWrittenPacketId = 0, I._lastReceivedPacketId = 0, I._handleCompressedPacket = A, I._inflatedPacketsParser = new _((t) => {
      I.handlePacket(t);
    }, 4), I._inflatedPacketsParser._lastPacket = 0, I.packetParser = new _((t) => {
      I._handleCompressedPacket(t);
    }, 7), I.writeUncompressed = I.write, I.write = R;
    const N = BT();
    I.inflateQueue = N.createQueue(), I.deflateQueue = N.createQueue();
  }
  return AA = {
    enableCompression: n
  }, AA;
}
const LT = DE, RA = CE, vE = G, hT = SE(), rR = YE;
function dR(E) {
  const _ = [];
  for (const A in vE)
    E & vE[A] && _.push(A.replace(/_/g, " ").toLowerCase());
  return _;
}
let UT = class U_ extends LT {
  constructor(_) {
    super(), this.handshake = null, this.clientFlags = _, this.authenticationFactor = 0;
  }
  start() {
    return U_.prototype.handshakeInit;
  }
  sendSSLRequest(_) {
    const A = new RA.SSLRequest(
      this.clientFlags,
      _.config.charsetNumber
    );
    _.writePacket(A.toPacket());
  }
  sendCredentials(_) {
    _.config.debug && console.log(
      "Sending handshake packet: flags:%d=(%s)",
      this.clientFlags,
      dR(this.clientFlags).join(", ")
    ...
    const A = new RA.HandshakeResponse({
  ...
  calculateNativePasswordAuthToken(_) {
    const A = _.slice(0, 8), R = _.slice(8, 20);
    let n;
    return this.passwordSha1 ? n = rR.calculateTokenFromPasswordSha(
      this.passwordSha1,
      A,
      R
    ) : n = rR.calculateToken(
  ...
  handshakeInit(_, A) {
    this.on("error", (I) => {
      A._fatalError = I, A._protocolError = I;
    }), this.handshake = RA.Handshake.fromPacket(_), A.config.debug && console.log(
      "Server hello packet: capability flags:%d=(%s)",
      this.handshake.capabilityFlags,
      dR(this.handshake.capabilityFlags).join(", ")
    ), A.serverCapabilityFlags = this.handshake.capabilityFlags, A.serverEncoding = hT[this.handshake.characterSet], A.connectionId = this.handshake.connectionId;
    const R = this.handshake.capabilityFlags & vE.SSL, n = this.handshake.capabilityFlags & vE.MULTI_FACTOR_AUTHENTICATION;
    if (this.clientFlags = this.clientFlags | n, A.config.compress = A.config.compress && this.handshake.capabilityFlags & vE.COMPRESS, this.clientFlags = this.clientFlags | A.config.compress, A.config.ssl) {
      if (!R) {
        const I = new Error("Server does not support secure connection");
        return I.code = "HANDSHAKE_NO_SSL_SUPPORT", I.fatal = !0, this.emit("error", I), !1;
      }
      this.clientFlags |= vE.SSL, this.sendSSLRequest(A), A.startTLS((I) => {
        if (I) {
          I.code = "HANDSHAKE_SSL_ERROR", I.fatal = !0, this.emit("error", I);
          return;
        }
      ...
      this.sendCredentials(A);
    return n && (this.authenticationFactor = 1), U_.prototype.handshakeResult;
  }
  handshakeResult(_, A) {
    const R = _.peekByte();
    if (R === 254 || R === 1 || R === 2) {
      const n = ST();
      try {
        return R === 1 ? n.authSwitchRequestMoreData(_, A, this) : (this.authenticationFactor !== 0 && (A.config.password = this[`password${this.authenticationFactor}`], this.authenticationFactor += 1), n.authSwitchRequest(_, A, this)), U_.prototype.handshakeResult;
      } catch (I) {
        return I.code = "AUTH_SWITCH_PLUGIN_ERROR", I.fatal = !0, this.onResult ? this.onResult(I) : this.emit("error", I), null;
      }
    }
    if (R !== 0) {
...
var ln = UT;
const PE = uE, u_ = FA, gT = DE, nA = CE;
let lT = class g_ extends gT {
  constructor(_) {
    super(), this.args = _;
  }
  start(_, A) {
    const R = new nA.Handshake(this.args);
    return this.serverHello = R, R.setScrambleData((n) => {
      if (n) {
        A.emit("error", new Error("Error generating random bytes"));
        return;
      }
      A.writePacket(R.toPacket(0));
    }), g_.prototype.readClientReply;
  }
  readClientReply(_, A) {
    const R = nA.HandshakeResponse.fromPacket(_);
    return A.clientHelloReply = R, this.args.authCallback ? this.args.authCallback(
      {
        user: R.user,
        database: R.database,
    ...
    ) : A.writeOk(), g_.prototype.dispatchCommands;
  }
  _isStatement(_, A) {
    return _.split(" ")[0].toUpperCase() === A;
  }
  dispatchCommands(_, A) {
    let R = !0;
    const n = A.clientHelloReply.encoding, I = _.readInt8();
    switch (I) {
      case PE.STMT_PREPARE:
        if (A.listeners("stmt_prepare").length) {
          const N = _.readString(void 0, n);
          A.emit("stmt_prepare", N);
        } else
          A.writeError({
            code: u_.HA_ERR_INTERNAL_ERROR,
            message: "No query handler for prepared statements."
          });
        break;
      case PE.STMT_EXECUTE:
        if (A.listeners("stmt_execute").length) {
          const { stmtId: N, flags: t, iterationCount: i, values: e } = nA.Execute.fromPacket(_, n);
          A.emit(
            "stmt_execute",
            N,
            t,
      ...
      case PE.QUERY:
        if (A.listeners("query").length) {
          const N = _.readString(void 0, n);
          this._isStatement(N, "PREPARE") || this._isStatement(N, "SET") ? A.emit("stmt_prepare", N) : this._isStatement(N, "EXECUTE") ? A.emit("stmt_execute", null, null, null, null, N) : A.emit("query", N);
        } else
          A.writeError({
            code: u_.HA_ERR_INTERNAL_ERROR,
      ...
      case PE.FIELD_LIST:
        if (A.listeners("field_list").length) {
          const N = _.readNullTerminatedString(n), t = _.readString(void 0, n);
          A.emit("field_list", N, t);
        } else
          A.writeError({
            code: u_.ER_WARN_DEPRECATED_SYNTAX,
    ...
    return A.listeners("packet").length ? A.emit("packet", _.clone(), R, I) : R || console.log("Unknown command:", I), g_.prototype.dispatchCommands;
  }
};
var sT = lT, IA = {}, GR;
function ZE() {
...
let hA, wR = !1;
try {
  hA = fT("cardinal").highlight;
} catch {
  hA = (_) => (wR || (console.log("For nicer debug output consider install cardinal@^2.0.0"), wR = !0), _);
}
function rT(E, _) {
  console.log(`
...
gE.printDebugWithCode = rT;
function dT(E, _, A) {
  return Array.isArray(_) ? _.some((R) => E === A[R]) : !!_;
}
gE.typeMatch = dT;
...
var FT = wT, FR = Cn, rn = FT, QR = /[\{\[]/, PR = /[\}\]]/, VR = [
  "do",
  "if",
  "in",
  "for",
...
for (var NA = 0; NA < VR.length; NA++)
  dn[VR[NA]] = !0;
var QT = function(E) {
  return rn(E) && !dn.hasOwnProperty(E);
}, Gn = {
  s: function(E) {
    return "" + E;
  },
  ...
  var E = [], _ = 0, A = {}, R = function(N) {
    for (var t = ""; t.length < _ * 2; ) t += "  ";
    E.push(t + N);
  }, n = function(N) {
    if (PR.test(N.trim()[0]) && QR.test(N[N.length - 1])) {
      _--, R(N), _++;
      return;
    }
    if (QR.test(N[N.length - 1])) {
      R(N), _++;
      return;
    }
    if (PR.test(N.trim()[0])) {
    ...
    if (!N) return I;
    if (arguments.length === 1 && N.indexOf(`
`) > -1)
      for (var t = N.trim().split(`
`), i = 0; i < t.length; i++)
        n(t[i].trim());
    else
      n(FR.format.apply(FR, arguments));
    return I;
  };
  return I.scope = {}, I.formats = Gn, I.sym = function(N) {
    return (!N || !QT(N)) && (N = "tmp"), A[N] || (A[N] = 0), N + (A[N]++ || "");
  }, I.property = function(N, t) {
    return arguments.length === 1 && (t = N, N = ""), t = t + "", rn(t) ? N ? N + "." + t : t : N ? N + "[" + JSON.stringify(t) + "]" : JSON.stringify(t);
  }, I.toString = function() {
    return E.join(`
`);
  }, I.toFunction = function(N) {
    N || (N = {});
    var t = "return (" + I.toString() + ")";
    Object.keys(I.scope).forEach(function(T) {
      N[T] || (N[T] = I.scope[T]);
    });
    ...
    return Function.apply(null, i.concat(t)).apply(null, e);
  }, arguments.length && I.apply(null, arguments), I;
};
wn.formats = Gn;
var Fn = wn;
const H = UE(), PT = ZE(), hE = gE, VT = Fn, bT = wA, Qn = [];
for (const E in H)
  Qn[H[E]] = E;
function oT(E, _, A, R, n) {
  const I = !!(n.supportBigNumbers || R.supportBigNumbers), N = !!(n.bigNumberStrings || R.bigNumberStrings), t = n.timezone || R.timezone, i = n.dateStrings || R.dateStrings;
  switch (E) {
    case H.TINY:
    case H.SHORT:
    case H.LONG:
    ...
    case H.LONGLONG:
      return I && N ? "packet.parseLengthCodedIntString()" : `packet.parseLengthCodedInt(${I})`;
    case H.FLOAT:
    case H.DOUBLE:
    ...
    case H.DECIMAL:
    case H.NEWDECIMAL:
      return R.decimalNumbers ? "packet.parseLengthCodedFloat()" : 'packet.readLengthCodedString("ascii")';
    case H.DATE:
      return hE.typeMatch(E, i, H) ? 'packet.readLengthCodedString("ascii")' : `packet.parseDate(${hE.srcEscape(t)})`;
    case H.DATETIME:
    case H.TIMESTAMP:
      return hE.typeMatch(E, i, H) ? 'packet.readLengthCodedString("ascii")' : `packet.parseDateTime(${hE.srcEscape(t)})`;
    case H.TIME:
    ...
    case H.JSON:
      return R.jsonStrings ? 'packet.readLengthCodedString("utf8")' : 'JSON.parse(packet.readLengthCodedString("utf8"))';
    default:
      return _ === PT.BINARY ? "packet.readLengthCodedBuffer()" : `packet.readLengthCodedString(${A})`;
  }
}
function YT(E, _, A) {
  typeof A.typeCast == "function" && typeof _.typeCast != "function" && (_.typeCast = A.typeCast);
  function R(T, a) {
    return {
      type: Qn[T.columnType],
      length: T.columnLength,
      db: T.schema,
      table: T.table,
      name: T.name,
      string: function(u = T.encoding) {
        return T.columnType === H.JSON && u === T.encoding && console.warn(
  ...
  const n = VT();
  n("(function () {")("return class TextRow {"), n("constructor(fields) {"), typeof _.typeCast == "function" && (n("const _this = this;"), n("for(let i=0; i<fields.length; ++i) {"), n("this[`wrap${i}`] = wrap(fields[i], _this);"), n("}")), n("}"), n("next(packet, fields, options) {"), n("this.packet = packet;"), _.rowsAsArray ? n(`const result = new Array(${E.length});`) : n("const result = {};");
  const I = {};
  let N = [];
  if (_.nestTables === !0) {
    ...
    N = Object.keys(I);
    for (let T = 0; T < N.length; T++)
      n(`result[${hE.fieldEscape(N[T])}] = {};`);
  }
  let t = "", i = "", e = "";
  for (let T = 0; T < E.length; T++)
    if (i = hE.fieldEscape(E[T].name), typeof _.nestTables == "string" ? t = `result[${hE.fieldEscape(E[T].table + _.nestTables + E[T].name)}]` : _.nestTables === !0 ? (e = hE.fieldEscape(E[T].table), n(`if (!result[${e}]) result[${e}] = {};`), t = `result[${e}][${i}]`) : _.rowsAsArray ? t = `result[${T.toString(10)}]` : t = `result[${i}]`, _.typeCast === !1)
      n(`${t} = packet.readLengthCodedBuffer();`);
    else {
      const a = `fields[${T}].encoding`, u = oT(
        E[T].columnType,
        E[T].characterSet,
        a,
        A,
      ...
      typeof _.typeCast == "function" ? n(
        `${t} = options.typeCast(this.wrap${T}, function() { return ${u} });`
      ) : n(`${t} = ${u};`);
    }
  return n("return result;"), n("}"), n("};")("})()"), A.debug && hE.printDebugWithCode(
    "Compiled text protocol row parser",
    n.toString()
  ), typeof _.typeCast == "function" ? n.toFunction({ wrap: R }) : n.toFunction();
}
function mT(E, _, A) {
  return bT.getParser("text", E, _, A, YT);
}
var WT = mT;
const y = UE(), HT = ZE(), bE = gE, Pn = [];
for (const E in y)
  Pn[y[E]] = E;
function yT({ packet: E, type: _, charset: A, encoding: R, config: n, options: I }) {
  const N = !!(I.supportBigNumbers || n.supportBigNumbers), t = !!(I.bigNumberStrings || n.bigNumberStrings), i = I.timezone || n.timezone, e = I.dateStrings || n.dateStrings;
  switch (_) {
    case y.TINY:
    case y.SHORT:
    case y.LONG:
    ...
    case y.LONGLONG:
      return N && t ? E.parseLengthCodedIntString() : E.parseLengthCodedInt(N);
    case y.FLOAT:
    case y.DOUBLE:
    ...
    case y.DATE:
      return bE.typeMatch(_, e, y) ? E.readLengthCodedString("ascii") : E.parseDate(i);
    case y.DATETIME:
    case y.TIMESTAMP:
      return bE.typeMatch(_, e, y) ? E.readLengthCodedString("ascii") : E.parseDateTime(i);
    case y.TIME:
    ...
    default:
      return A === HT.BINARY ? E.readLengthCodedBuffer() : E.readLengthCodedString(R);
  }
}
function vT(E, _) {
  return {
    type: Pn[E.columnType],
    length: E.columnLength,
    db: E.schema,
    table: E.table,
    name: E.name,
    string: function(A = E.encoding) {
      return E.columnType === y.JSON && A === E.encoding && console.warn(
...
function XT(E, _, A) {
  return {
    next(R, n, I) {
      const N = I.rowsAsArray ? [] : {};
      for (let t = 0; t < n.length; t++) {
        const i = n[t], e = I.typeCast ? I.typeCast : A.typeCast, T = () => yT({
          packet: R,
          type: i.columnType,
          encoding: i.encoding,
        ...
        let a;
        if (I.typeCast === !1 ? a = R.readLengthCodedBuffer() : typeof e == "function" ? a = e(vT(i, R), T) : a = T(), I.rowsAsArray)
          N.push(a);
        else if (typeof I.nestTables == "string")
          N[`${bE.fieldEscape(i.table, !1)}${I.nestTables}${bE.fieldEscape(i.name, !1)}`] = a;
        else if (I.nestTables) {
          const u = bE.fieldEscape(i.table, !1);
          N[u] || (N[u] = {}), N[u][bE.fieldEscape(i.name, !1)] = a;
        } else
          N[bE.fieldEscape(i.name, !1)] = a;
      }
      return N;
    }
  };
...
var KT = XT;
const bR = d_, TA = dA, jT = an.Readable, JT = DE, WE = CE, zT = WT, ZT = KT, oR = AE, eA = new WE.Packet(0, Buffer.allocUnsafe(4), 0, 4);
let UA = class HE extends JT {
  constructor(_, A) {
  ...
  start(_, A) {
    A.config.debug && console.log("        Sending query command: %s", this.sql), this._connection = A, this.options = Object.assign({}, A.config, this._queryOptions), this._setTimeout();
    const R = new WE.Query(
      this.sql,
      A.config.charsetNumber
    );
    return A.writePacket(R.toPacket(1)), HE.prototype.resultsetHeader;
  }
  done() {
    if (this._unpipeStream(), this.timeout && !this.queryTimeout)
      return null;
    if (this.queryTimeout && (TA.clearTimeout(this.queryTimeout), this.queryTimeout = null), this.onResult) {
      let _, A;
      this._resultIndex === 0 ? (_ = this._rows[0], A = this._fields[0]) : (_ = this._rows, A = this._fields), A ? bR.nextTick(() => {
        this.onResult(null, _, A);
      }) : bR.nextTick(() => {
  ...
  doneInsert(_) {
    return this._localStreamError ? (this.onResult ? this.onResult(this._localStreamError, _) : this.emit("error", this._localStreamError), null) : (this._rows.push(_), this._fields.push(void 0), this.emit("fields", void 0), this.emit("result", _), _.serverStatus & oR.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, this.resultsetHeader) : this.done());
  }
  resultsetHeader(_, A) {
    const R = new WE.ResultSetHeader(_, A);
    return this._fieldCount = R.fieldCount, A.config.debug && console.log(
      `        Resultset header received, expecting ${R.fieldCount} column definition packets`
    ), this._fieldCount === 0 ? this.doneInsert(R) : this._fieldCount === null ? this._streamLocalInfile(A, R.infileName) : (this._receivedFieldsCount = 0, this._rows.push([]), this._fields.push([]), this.readField);
  }
  _streamLocalInfile(_, A) {
    if (this._streamFactory)
      this._localStream = this._streamFactory(A);
    else
      return this._localStreamError = new Error(
        `As a result of LOCAL INFILE command server wants to read ${A} file, but as of v2.0 you must provide streamFactory option returning ReadStream.`
      ), _.writePacket(eA), this.infileOk;
    const R = () => {
      this._unpipeStream();
    }, n = () => {
      ...
      const T = Buffer.allocUnsafe(e.length + 4);
      e.copy(T, 4), _.writePacket(
        new WE.Packet(0, T, 0, T.length)
      );
    }, t = () => {
      _.removeListener("error", R), _.writePacket(eA);
    }, i = (e) => {
      this._localStreamError = e, _.removeListener("error", R), _.writePacket(eA);
    };
    return this._unpipeStream = () => {
      _.stream.removeListener("pause", I), _.stream.removeListener("drain", n), this._localStream.removeListener("data", N), this._localStream.removeListener("end", t), this._localStream.removeListener("error", i);
    }, _.stream.on("pause", I), _.stream.on("drain", n), this._localStream.on("data", N), this._localStream.on("end", t), this._localStream.on("error", i), _.once("error", R), this.infileOk;
  }
  readField(_, A) {
    if (this._receivedFieldsCount++, this._fields[this._resultIndex].length !== this._fieldCount) {
      const R = new WE.ColumnDefinition(
        _,
        A.clientEncoding
      );
      this._fields[this._resultIndex].push(R), A.config.debug && (console.log("        Column definition:"), console.log(`          name: ${R.name}`), console.log(`          type: ${R.columnType}`), console.log(`         flags: ${R.flags}`));
    }
    if (this._receivedFieldsCount === this._fieldCount) {
      const R = this._fields[this._resultIndex];
      return this.emit("fields", R), this.options.disableEval ? this._rowParser = ZT(R, this.options, A.config) : this._rowParser = new (zT(
        R,
        this.options,
        A.config
      ))(R), HE.prototype.fieldsEOF;
    }
    return HE.prototype.readField;
  }
  fieldsEOF(_, A) {
    return _.isEOF() ? this.row : A.protocolError("Expected EOF packet");
  }
  /* eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }] */
  row(_, A) {
    if (_.isEOF())
      return _.eofStatusFlags() & oR.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, HE.prototype.resultsetHeader) : this.done();
    let R;
    try {
      R = this._rowParser.next(
        _,
        this._fields[this._resultIndex],
        this.options
      );
    } catch (n) {
      return this._localStreamError = n, this.doneInsert(null);
    }
    return this.onResult ? this._rows[this._resultIndex].push(R) : this.emit("result", R, this._resultIndex), HE.prototype.row;
  }
  infileOk(_, A) {
    const R = new WE.ResultSetHeader(_, A);
    return this.doneInsert(R);
  }
  stream(_) {
    ...
    A.once("close", () => {
      A.readableEnded || A.emit("end");
    });
    const R = (t, i) => {
      A.destroyed || (A.push(t) || this._connection && this._connection.pause(), A.emit("result", t, i));
    }, n = (t) => {
      A.destroyed || A.emit("fields", t);
    }, I = () => {
      A.destroyed || A.push(null);
    }, N = (t) => {
      A.destroy(t);
    };
    return A._destroy = (t, i) => {
      this._connection && this._connection.resume(), this.removeListener("result", R), this.removeListener("fields", n), this.removeListener("end", I), this.removeListener("error", N), i(t);
    }, this.on("result", R), this.on("fields", n), this.on("end", I), this.on("error", N), A;
  }
  _setTimeout() {
    if (this.timeout) {
      const _ = this._handleTimeoutError.bind(this);
      this.queryTimeout = TA.setTimeout(_, this.timeout);
    }
  }
  _handleTimeoutError() {
    this.queryTimeout && (TA.clearTimeout(this.queryTimeout), this.queryTimeout = null);
...
UA.prototype.catch = UA.prototype.then;
var Vn = UA;
const kT = DE, qT = CE;
let pT = class extends kT {
  ...
  start(_, A) {
    return A.writePacket(new qT.CloseStatement(this.id).toPacket(1)), null;
  }
};
var xT = pT;
const bn = q, $T = ZE(), b = UE(), XE = gE, Ee = Fn, _e = wA, on = [];
for (const E in b)
  on[b[E]] = E;
function Ae(E, _, A, R) {
  const n = !!(A.supportBigNumbers || _.supportBigNumbers), I = !!(A.bigNumberStrings || _.bigNumberStrings), N = A.timezone || _.timezone, t = A.dateStrings || _.dateStrings, i = E.flags & bn.UNSIGNED;
  switch (E.columnType) {
    case b.TINY:
    ...
    case b.TIMESTAMP:
    case b.NEWDATE:
      return XE.typeMatch(E.columnType, t, b) ? `packet.readDateTimeString(${parseInt(E.decimals, 10)}, null, ${E.columnType});` : `packet.readDateTime(${XE.srcEscape(N)});`;
    case b.TIME:
    ...
    case b.LONGLONG:
      return n ? I ? i ? "packet.readInt64String();" : "packet.readSInt64String();" : i ? "packet.readInt64();" : "packet.readSInt64();" : i ? "packet.readInt64JSNumber();" : "packet.readSInt64JSNumber();";
    default:
      return E.characterSet === $T.BINARY ? "packet.readLengthCodedBuffer();" : `packet.readLengthCodedString(fields[${R}].encoding)`;
  }
}
function Re(E, _, A) {
  const R = Ee(), n = Math.floor((E.length + 7 + 2) / 8);
  function I(a, u) {
    return {
      type: on[a.columnType],
      length: a.columnLength,
      db: a.schema,
      table: a.table,
      name: a.name,
      string: function(S = a.encoding) {
        if (a.columnType === b.JSON && S === a.encoding && console.warn(
        ...
        ), [b.DATETIME, b.NEWDATE, b.TIMESTAMP, b.DATE].includes(
          a.columnType
        ))
        ...
        if (a.columnType === b.TINY) {
          const O = a.flags & bn.UNSIGNED;
          return String(O ? u.readInt8() : u.readSInt8());
        }
        return a.columnType === b.TIME ? u.readTimeString() : u.readLengthCodedString(S);
      },
      buffer: function() {
  ...
  R("(function(){"), R("return class BinaryRow {"), R("constructor() {"), R("}"), R("next(packet, fields, options) {"), _.rowsAsArray ? R(`const result = new Array(${E.length});`) : R("const result = {};"), typeof A.typeCast == "function" && typeof _.typeCast != "function" && (_.typeCast = A.typeCast), R("packet.readInt8();");
  for (let a = 0; a < n; ++a)
    R(`const nullBitmaskByte${a} = packet.readInt8();`);
  let N = "", t = 4, i = 0, e = "", T = "";
  for (let a = 0; a < E.length; a++) {
    if (e = XE.fieldEscape(E[a].name), typeof _.nestTables == "string" ? N = `result[${XE.fieldEscape(E[a].table + _.nestTables + E[a].name)}]` : _.nestTables === !0 ? (T = XE.fieldEscape(E[a].table), R(`if (!result[${T}]) result[${T}] = {};`), N = `result[${T}][${e}]`) : _.rowsAsArray ? N = `result[${a.toString(10)}]` : N = `result[${e}]`, R(`if (nullBitmaskByte${i} & ${t}) `), R(`${N} = null;`), R("else {"), _.typeCast === !1)
      R(`${N} = packet.readLengthCodedBuffer();`);
    else {
      const u = `fieldWrapper${a}`;
      R(`const ${u} = wrap(fields[${a}], packet);`);
      const S = Ae(E[a], A, _, a);
      typeof _.typeCast == "function" ? R(
        `${N} = options.typeCast(${u}, function() { return ${S} });`
      ) : R(`${N} = ${S};`);
    }
    R("}"), t *= 2, t === 256 && (t = 1, i++);
  }
  return R("return result;"), R("}"), R("};")("})()"), A.debug && XE.printDebugWithCode(
    "Compiled binary protocol row parser",
    R.toString()
  ), R.toFunction({ wrap: I });
}
function ne(E, _, A) {
  return _e.getParser("binary", E, _, A, Re);
}
var Ie = ne;
const YR = q, Ne = ZE(), V = UE(), qE = gE, Yn = [];
for (const E in V)
  Yn[V[E]] = E;
function Te(E, _, A) {
  function R(n, I, N, t, i) {
    const e = !!(N.supportBigNumbers || I.supportBigNumbers), T = !!(N.bigNumberStrings || I.bigNumberStrings), a = N.timezone || I.timezone, u = N.dateStrings || I.dateStrings, S = n.flags & YR.UNSIGNED;
    switch (n.columnType) {
      case V.TINY:
        return S ? i.readInt8() : i.readSInt8();
      case V.SHORT:
        return S ? i.readInt16() : i.readSInt16();
      case V.LONG:
      case V.INT24:
        return S ? i.readInt32() : i.readSInt32();
      case V.YEAR:
        return i.readInt16();
      case V.FLOAT:
        return i.readFloat();
      ...
      case V.TIMESTAMP:
      case V.NEWDATE:
        return qE.typeMatch(n.columnType, u, V) ? i.readDateTimeString(
      ...
      case V.DECIMAL:
      case V.NEWDECIMAL:
        return I.decimalNumbers ? i.parseLengthCodedFloat() : i.readLengthCodedString("ascii");
      case V.GEOMETRY:
        return i.parseGeometryValue();
      case V.VECTOR:
        return i.parseVector();
      case V.JSON:
        return I.jsonStrings ? i.readLengthCodedString("utf8") : JSON.parse(i.readLengthCodedString("utf8"));
      case V.LONGLONG:
        return e ? T ? S ? i.readInt64String() : i.readSInt64String() : S ? i.readInt64() : i.readSInt64() : S ? i.readInt64JSNumber() : i.readSInt64JSNumber();
      default:
        return n.characterSet === Ne.BINARY ? i.readLengthCodedBuffer() : i.readLengthCodedString(E[t].encoding);
    }
  }
  return class {
    constructor() {
    ...
    next(I, N, t) {
      I.readInt8();
      const i = Math.floor((N.length + 7 + 2) / 8), e = new Array(i);
      for (let S = 0; S < i; S++)
        e[S] = I.readInt8();
      const T = t.rowsAsArray ? new Array(N.length) : {};
      let a = 4, u = 0;
      for (let S = 0; S < N.length; S++) {
        const O = N[S], D = t.typeCast !== void 0 ? t.typeCast : A.typeCast;
        let c;
        if (e[u] & a)
          c = null;
        else if (t.typeCast === !1)
          c = I.readLengthCodedBuffer();
        else {
          const h = () => R(O, A, t, S, I);
          c = typeof D == "function" ? D(
            {
              type: Yn[O.columnType],
              length: O.columnLength,
              db: O.schema,
              table: O.table,
              name: O.name,
              string: function(M = O.encoding) {
                if (O.columnType === V.JSON && M === O.encoding && console.warn(
                ...
                if (O.columnType === V.TINY) {
                  const L = O.flags & YR.UNSIGNED;
                  return String(
                    L ? I.readInt8() : I.readSInt8()
                  );
                }
                return O.columnType === V.TIME ? I.readTimeString() : I.readLengthCodedString(M);
              },
              buffer: function() {
        ...
        if (t.rowsAsArray)
          ...
          const h = qE.fieldEscape(
            O.table + t.nestTables + O.name,
            !1
          );
          T[h] = c;
        } else if (t.nestTables === !0) {
          const h = qE.fieldEscape(O.table, !1);
          T[h] || (T[h] = {});
          const M = qE.fieldEscape(O.name, !1);
          T[h][M] = c;
        } else {
          const h = qE.fieldEscape(O.name, !1);
          T[h] = c;
        }
        a *= 2, a === 256 && (a = 1, u++);
      }
...
var ee = Te;
const te = DE, lE = Vn, mR = CE, ie = Ie, ae = ee;
let cE = class __ extends te {
  constructor(_, A) {
    super(), this.statement = _.statement, this.sql = _.sql, this.values = _.values, this.onResult = A, this.parameters = _.values, this.insertId = 0, this.timeout = _.timeout, this.queryTimeout = null, this._rows = [], this._fields = [], this._result = [], this._fieldCount = 0, this._rowParser = null, this._executeOptions = _, this._resultIndex = 0, this._localStream = null, this._unpipeStream = function() {
    }, this._streamFactory = _.infileStreamFactory, this._connection = null;
  }
  buildParserFromFields(_, A) {
    return this.options.disableEval ? ae(_, this.options, A.config) : ie(_, this.options, A.config);
  }
  start(_, A) {
    this._connection = A, this.options = Object.assign({}, A.config, this._executeOptions), this._setTimeout();
    const R = new mR.Execute(
      this.statement.id,
      this.parameters,
      A.config.charsetNumber,
      A.config.timezone
    ...
    try {
      A.writePacket(R.toPacket(1));
    } catch (n) {
      this.onResult(n);
    }
    return __.prototype.resultsetHeader;
  }
  readField(_, A) {
    let R;
    const n = new mR.ColumnDefinition(
      _,
      A.clientEncoding
    );
    return this._receivedFieldsCount++, this._fields[this._resultIndex].push(n), this._receivedFieldsCount === this._fieldCount ? (R = this._fields[this._resultIndex], this.emit("fields", R, this._resultIndex), __.prototype.fieldsEOF) : __.prototype.readField;
  }
  fieldsEOF(_, A) {
    return _.isEOF() ? (this._rowParser = new (this.buildParserFromFields(
      this._fields[this._resultIndex],
      A
    ))(), __.prototype.row) : A.protocolError("Expected EOF packet");
  }
};
cE.prototype.done = lE.prototype.done;
cE.prototype.doneInsert = lE.prototype.doneInsert;
cE.prototype.resultsetHeader = lE.prototype.resultsetHeader;
cE.prototype._findOrCreateReadStream = lE.prototype._findOrCreateReadStream;
cE.prototype._streamLocalInfile = lE.prototype._streamLocalInfile;
cE.prototype._setTimeout = lE.prototype._setTimeout;
cE.prototype._handleTimeoutError = lE.prototype._handleTimeoutError;
cE.prototype.row = lE.prototype.row;
cE.prototype.stream = lE.prototype.stream;
var mn = cE;
const S_ = CE, Oe = DE, Ce = xT, De = mn;
class ue {
  constructor(_, A, R, n, I) {
    this.query = _, this.id = A, this.columns = R, this.parameters = n, this.rowParser = null, this._connection = I;
  }
  close() {
    return this._connection.addCommand(new Ce(this.id));
  }
  execute(_, A) {
    return typeof _ == "function" && (A = _, _ = []), this._connection.addCommand(
      new De({ statement: this, values: _ }, A)
    );
  }
}
let Se = class LE extends Oe {
  constructor(_, A) {
    super(), this.query = _.sql, this.onResult = A, this.id = 0, this.fieldCount = 0, this.parameterCount = 0, this.fields = [], this.parameterDefinitions = [], this.options = _;
  }
  start(_, A) {
    ...
    const I = new S_.PrepareStatement(
      this.query,
      A.config.charsetNumber,
      this.options.values
    );
    return A.writePacket(I.toPacket(1)), LE.prototype.prepareHeader;
  }
  prepareHeader(_, A) {
    const R = new S_.PreparedStatementHeader(_);
    return this.id = R.id, this.fieldCount = R.fieldCount, this.parameterCount = R.parameterCount, this.parameterCount > 0 ? LE.prototype.readParameter : this.fieldCount > 0 ? LE.prototype.readField : this.prepareDone(A);
  }
  readParameter(_, A) {
    if (_.isEOF())
      return this.fieldCount > 0 ? LE.prototype.readField : this.prepareDone(A);
    const R = new S_.ColumnDefinition(_, A.clientEncoding);
    return this.parameterDefinitions.push(R), this.parameterDefinitions.length === this.parameterCount ? LE.prototype.parametersEOF : this.readParameter;
  }
  readField(_, A) {
    if (_.isEOF())
      return this.prepareDone(A);
    const R = new S_.ColumnDefinition(_, A.clientEncoding);
    return this.fields.push(R), this.fields.length === this.fieldCount ? LE.prototype.fieldsEOF : LE.prototype.readField;
  }
  parametersEOF(_, A) {
    return _.isEOF() ? this.fieldCount > 0 ? LE.prototype.readField : this.prepareDone(A) : A.protocolError("Expected EOF packet after parameters");
  }
  fieldsEOF(_, A) {
    return _.isEOF() ? this.prepareDone(A) : A.protocolError("Expected EOF packet after fields");
  }
  prepareDone(_) {
    const A = new ue(
      this.query,
    ...
    return _._statements.set(this.key, A), this.onResult && this.onResult(null, A), null;
  }
};
var ce = Se;
const Be = DE, Me = uE, Le = K;
let he = class Wn extends Be {
  constructor(_) {
  ...
  start(_, A) {
    const R = new Le(
      0,
      Buffer.from([1, 0, 0, 0, Me.PING]),
      0,
      5
    );
    return A.writePacket(R), Wn.prototype.pingResponse;
  }
  pingResponse() {
...
var Ue = he;
const ge = DE, le = CE;
let se = class Hn extends ge {
  constructor(_, A) {
    super(), this.onResult = A, this.opts = _;
  }
  start(_, A) {
    const R = new le.RegisterSlave(this.opts);
    return A.writePacket(R.toPacket(1)), Hn.prototype.registerResponse;
  }
  registerResponse() {
    return this.onResult && process.nextTick(this.onResult.bind(this)), null;
  }
...
function re() {
  ...
  return tA = function(A) {
    ...
    for (; n < A.length; )
      switch (I = A[n++], I) {
        case E.FLAGS2:
          R.flags = A.readUInt32LE(n), n += 4;
          break;
        case E.SQL_MODE:
          R.sqlMode = A.readUInt32LE(n), n += 8;
          break;
        case E.CATALOG:
          N = A[n++], R.catalog = A.toString("utf8", n, n + N), n += N + 1;
          break;
        case E.CHARSET:
          R.clientCharset = A.readUInt16LE(n), R.connectionCollation = A.readUInt16LE(n + 2), R.serverCharset = A.readUInt16LE(n + 4), n += 6;
          break;
        case E.TIME_ZONE:
          N = A[n++], R.timeZone = A.toString("utf8", n, n + N), n += N;
          break;
        case E.CATALOG_NZ:
          N = A[n++], R.catalogNz = A.toString("utf8", n, n + N), n += N;
          break;
        case E.LC_TIME_NAMES:
          R.lcTimeNames = A.readUInt16LE(n), n += 2;
          break;
        case E.CHARSET_DATABASE:
          R.schemaCharset = A.readUInt16LE(n), n += 2;
          break;
        case E.TABLE_MAP_FOR_UPDATE:
          R.mapForUpdate1 = A.readUInt32LE(n), R.mapForUpdate2 = A.readUInt32LE(n + 4), n += 8;
          break;
        case E.MASTER_DATA_WRITTEN:
          R.masterDataWritten = A.readUInt32LE(n), n += 4;
          break;
        case E.INVOKERS:
          N = A[n++], R.invokerUsername = A.toString(
            "utf8",
            n,
            n + N
          ), n += N, N = A[n++], R.invokerHostname = A.toString(
            "utf8",
            n,
            n + N
          ), n += N;
        ...
        case E.UPDATED_DB_NAMES:
          for (N = A[n++], R.updatedDBs = []; N; --N) {
            for (t = n; A[n++] && n < A.length; )
              ;
            R.updatedDBs.push(
              A.toString("utf8", t, n - 1)
            );
          }
          break;
        case E.MICROSECONDS:
          R.microseconds = // REVIEW: INVALID UNKNOWN VARIABLE!
          A.readInt16LE(n) + (A[n + 2] << 16), n += 3;
      }
    return R;
  }, tA;
}
const de = DE, Ge = CE, I_ = [];
class we {
  constructor(_) {
...
let Fe = class gA extends de {
  constructor(_) {
    super(), this.opts = _;
  }
  start(_, A) {
    const R = new Ge.BinlogDump(this.opts);
    return A.writePacket(R.toPacket(1)), gA.prototype.binlogData;
  }
  binlogData(_) {
    if (_.isEOF())
      return this.emit("eof"), null;
    _.readInt8();
    const A = new we(_), R = I_[A.eventType];
    let n;
    return R ? n = new R(_) : n = {
      name: "UNKNOWN"
    }, n.header = A, this.emit("event", n), gA.prototype.binlogData;
  }
};
class Qe {
  constructor(_) {
...
class Ve {
  constructor(_) {
    ...
    const n = _.readInt16(), I = _.readBuffer(n);
    this.schema = _.readString(R), _.readInt8(), this.statusVars = A(I), this.query = _.readString(), this.name = "QueryEvent";
  }
}
class be {
  constructor(_) {
...
I_[16] = be;
var oe = Fe;
const Ye = DE, me = CE, We = G, yn = ln, He = SE();
let KA = class vn extends Ye {
  constructor(_, A) {
    super(), this.onResult = A, this.user = _.user, this.password = _.password, this.password1 = _.password, this.password2 = _.password2, this.password3 = _.password3, this.database = _.database, this.passwordSha1 = _.passwordSha1, this.charsetNumber = _.charsetNumber, this.currentConfig = _.currentConfig, this.authenticationFactor = 0;
  }
  start(_, A) {
    const R = new me.ChangeUser({
    ...
    return this.currentConfig.user = this.user, this.currentConfig.password = this.password, this.currentConfig.database = this.database, this.currentConfig.charsetNumber = this.charsetNumber, A.clientEncoding = He[this.charsetNumber], A._statements.clear(), A.writePacket(R.toPacket()), A.serverCapabilityFlags & We.MULTI_FACTOR_AUTHENTICATION && (this.authenticationFactor = 1), vn.prototype.handshakeResult;
  }
};
KA.prototype.handshakeResult = yn.prototype.handshakeResult;
KA.prototype.calculateNativePasswordAuthToken = yn.prototype.calculateNativePasswordAuthToken;
var ye = KA;
const ve = DE, Xe = uE, Ke = K;
let je = class extends ve {
  ...
  start(_, A) {
    ...
    const R = new Ke(
      0,
      Buffer.from([1, 0, 0, 0, Xe.QUIT]),
      0,
      5
    );
    return this.onResult && this.onResult(), A.writePacket(R), null;
  }
};
var Je = je;
const ze = ln, Ze = sT, ke = Vn, qe = ce, pe = mn, xe = Ue, $e = fe, Et = oe, _t = ye, At = Je;
var Rt = {
  ClientHandshake: ze,
  ServerHandshake: Ze,
  Query: ke,
...
var iA = {}, yE = { exports: {} }, pE = {}, HR;
function Nt() {
  return HR || (HR = 1, Object.defineProperty(pE, "__esModule", { value: !0 }), pE.defaults = void 0, pE.defaults = [
    `-----BEGIN CERTIFICATE-----
...
var xE = {}, yR;
function Tt() {
  return yR || (yR = 1, Object.defineProperty(xE, "__esModule", { value: !0 }), xE.proxies = void 0, xE.proxies = [
    `-----BEGIN CERTIFICATE-----
...
function tt() {
  ...
  return yE.exports = R, yE.exports.proxyBundle = A, yE.exports.default = R, yE.exports;
}
var XR;
function it() {
...
const { URL: at } = aI, KR = G, jR = ZE(), { version: Ot } = It;
let aA = null;
const Ct = {
  authPlugins: 1,
  authSwitchHandler: 1,
...
let Dt = class VE {
  constructor(_) {
    if (typeof _ == "string")
      ...
      const R = VE.parseUrl(_.uri);
      for (const n in R)
        Object.prototype.hasOwnProperty.call(R, n) && (_[n] || (_[n] = R[n]));
    }
    for (const R in _)
      Object.prototype.hasOwnProperty.call(_, R) && Ct[R] !== 1 && console.error(
        `Ignoring invalid configuration option passed to Connection: ${R}. This is currently a warning, but in future versions of MySQL2, an error will be thrown if you pass an invalid configuration option to a Connection`
      );
    ...
    ), this.timezone = "Z") : this.timezone = _.timezone || "local", this.queryFormat = _.queryFormat, this.pool = _.pool || void 0, this.ssl = typeof _.ssl == "string" ? VE.getSSLProfile(_.ssl) : _.ssl || !1, this.multipleStatements = _.multipleStatements || !1, this.rowsAsArray = _.rowsAsArray || !1, this.namedPlaceholders = _.namedPlaceholders || !1, this.nestTables = _.nestTables === void 0 ? void 0 : _.nestTables, this.typeCast = _.typeCast === void 0 ? !0 : _.typeCast, this.disableEval = !!_.disableEval, this.timezone[0] === " " && (this.timezone = `+${this.timezone.slice(1)}`), this.ssl) {
    ...
    this.maxPacketSize = 0, this.charsetNumber = _.charset ? VE.getCharsetNumber(_.charset) : _.charsetNumber || jR.UTF8MB4_UNICODE_CI, this.compress = _.compress || !1, this.authPlugins = _.authPlugins, this.authSwitchHandler = _.authSwitchHandler, this.clientFlags = VE.mergeFlags(
      VE.getDefaultFlags(_),
      _.flags || ""
    );
    const A = {
  ...
  static mergeFlags(_, A) {
    let R = 0, n;
    Array.isArray(A) || (A = String(A || "").toUpperCase().split(/\s*,+\s*/));
    for (n in _)
      A.indexOf(`-${_[n]}`) >= 0 || (R |= KR[_[n]] || 0);
    for (n in A)
      A[n][0] !== "-" && (_.indexOf(A[n]) >= 0 || (R |= KR[A[n]] || 0));
    return R;
  }
  static getDefaultFlags(_) {
    ...
    return _ && _.multipleStatements && A.push("MULTI_STATEMENTS"), A.push("PLUGIN_AUTH"), A.push("PLUGIN_AUTH_LENENC_CLIENT_DATA"), A;
  }
  static getCharsetNumber(_) {
    const A = jR[_.toUpperCase()];
    if (A === void 0)
  ...
  static parseUrl(_) {
    ...
    return A.searchParams.forEach((n, I) => {
      try {
        R[I] = JSON.parse(n);
      } catch {
        R[I] = n;
...
var jA = Dt, c_ = { exports: {} }, JR;
function ut() {
  if (JR) return c_.exports;
  JR = 1;
  const E = /(?:\?)|(?::(\d+|(?:[a-zA-Z][a-zA-Z0-9_]*)))/g, _ = 34, A = 39, R = 92;
  function n(t) {
    let i = E.exec(t), e = 0, T = 0, a;
    const u = [];
    let S = !1, O = !1, D;
    const c = [];
    let h = 0, M = 0, L;
      ...
      do {
        for (L = e, a = i.index; L < a; ++L) {
          ...
          if (B === R) O = !O;
            ...
            if (S && B === D) {
              if (t.charCodeAt(L + 1) === D) {
                ++L;
                continue;
              }
              S = !1;
            } else !S && (B === _ || B === A) && (S = !0, D = B);
          }
        }
        S || (u.push(t.substring(T, a)), c.push(i[0].length === 1 ? h++ : i[1]), T = a + i[0].length, M = T), e = a + i[0].length;
      } while (i = E.exec(t));
      if (c.length)
        return e < t.length && u.push(t.substring(M)), [u, c];
    }
    return [t];
  }
  function I(t) {
    t || (t = {}), t.placeholder || (t.placeholder = "?");
    let i = 100, e;
    typeof t.cache == "number" && (i = t.cache), typeof t.cache == "object" && (e = t.cache), t.cache !== !1 && !e && (e = oE.createLRU({ max: i }));
    function T(O, D) {
      const c = [];
      if (O.length === 1)
        return [O[0], []];
    ...
    function u(O) {
      if (O.length === 1)
        return O;
      let D = a(O[0][0]);
      for (let h = 1; h < O[0].length; ++h)
        O[0][h - 1].slice(-1) === ":" && (D += t.placeholder), D += t.placeholder, D += a(O[0][h]);
      const c = O[0][O[0].length - 1];
      return O[0].length === O[1].length && (c.slice(-1) === ":" && (D += t.placeholder), D += t.placeholder), [D, O[1]];
    }
    function S(O, D) {
      let c;
      return e && (c = e.get(O)) || (c = u(n(O)), e && e.set(O, c)), T(c, D);
    }
    return S.parse = n, S;
  }
  function N(t, i) {
    ...
    for (let c = 0; c < e[0].length; ++c)
      O = a[e[1][c]], O || (O = ++u, a[e[1][c]] = O), e[1][c] ? (D[O - 1] = e[1][c], S += `${e[0][c]}$${O}`) : S += e[0][c];
    return [S, D.map((c) => i[c])];
  }
  return c_.exports = I, c_.exports.toNumbered = N, c_.exports;
}
const OA = eI, B_ = tn, mE = dA, zR = GE.EventEmitter, St = an.Readable, ZR = cn, M_ = GA, { createLRU: ct } = oE, Bt = Ln, sE = CE, NE = Rt, Mt = jA, kR = SE();
let Lt = 0, CA = null, ht = class lA extends zR {
  constructor(_) {
    super(), this.config = _.config, _.config.stream ? typeof _.config.stream == "function" ? this.stream = _.config.stream(_) : this.stream = _.config.stream : _.config.socketPath ? this.stream = OA.connect(_.config.socketPath) : (this.stream = OA.connect(_.config.port, _.config.host), this.config.enableKeepAlive && this.stream.on("connect", () => {
      this.stream.setKeepAlive(!0, this.config.keepAliveInitialDelay);
    }), this.stream.setNoDelay(!0)), this._internalId = Lt++, this._commands = new ZR(), this._command = null, this._paused = !1, this._paused_packets = new ZR(), this._statements = ct({
      max: this.config.maxPreparedStatements,
      onEviction: function(R, n) {
        n.close();
      }
    }), this.serverCapabilityFlags = 0, this.authorized = !1, this.sequenceId = 0, this.compressedSequenceId = 0, this.threadId = null, this._handshakePacket = null, this._fatalError = null, this._protocolError = null, this._outOfOrderPackets = [], this.clientEncoding = kR[this.config.charsetNumber], this.stream.on("error", this._handleNetworkError.bind(this)), this.packetParser = new Bt((R) => {
      this.handlePacket(R);
    }), this.stream.on("data", (R) => {
      this.connectTimeout && (mE.clearTimeout(this.connectTimeout), this.connectTimeout = null), this.packetParser.execute(R);
    }), this.stream.on("end", () => {
      this.emit("end");
    ...
    let A;
    if (this.config.isServer || (A = new NE.ClientHandshake(this.config.clientFlags), A.on("end", () => {
      !A.handshake || this._fatalError || this._protocolError || (this._handshakePacket = A.handshake, this.threadId = A.handshake.connectionId, this.emit("connect", A.handshake));
    }), A.on("error", (R) => {
      this._closing = !0, this._notifyError(R);
    }), this.addCommand(A)), this.serverEncoding = "utf8", this.config.connectTimeout) {
      const R = this._handleTimeoutError.bind(this);
      this.connectTimeout = mE.setTimeout(
        R,
  ...
  _addCommandClosedState(_) {
    ...
    A.fatal = !0, _.onResult ? _.onResult(A) : this.emit("error", A);
  }
  _handleFatalError(_) {
  ...
  _handleNetworkError(_) {
    this.connectTimeout && (mE.clearTimeout(this.connectTimeout), this.connectTimeout = null), !(_.code === "ECONNRESET" && this._closing) && this._handleFatalError(_);
  }
  _handleTimeoutError() {
    this.connectTimeout && (mE.clearTimeout(this.connectTimeout), this.connectTimeout = null), this.stream.destroy && this.stream.destroy();
  ...
  _notifyError(_) {
    if (this.connectTimeout && (mE.clearTimeout(this.connectTimeout), this.connectTimeout = null), this._fatalError)
      return;
    let A, R = !this._command;
    for (this._command && this._command.onResult ? (this._command.onResult(_), this._command = null) : this._command && this._command.constructor === NE.ClientHandshake && this._commands.length > 0 || (R = !0); A = this._commands.shift(); )
      A.onResult ? A.onResult(_) : R = !0;
    (R || this._pool) && this.emit("error", _), _.fatal && this.close();
  }
  write(_) {
    this.stream.write(_, (R) => {
  ...
  writePacket(_) {
    ...
    if (R < 16777215)
      ...
      for (this.config.debug && (console.log(
        ...
        `${this._internalId} ${this.connectionId} <== ${this._command._commandName}#${this._command.stateName()}(${[this.sequenceId, _._name, _.length()].join(",")})`
      )), I = 4; I < 4 + R; I += 16777215)
        n = _.buffer.slice(I, I + 16777215), n.length === 16777215 ? N = Buffer.from([255, 255, 255, this.sequenceId]) : N = Buffer.from([
          n.length & 255,
          n.length >> 8 & 255,
          n.length >> 16 & 255,
          this.sequenceId
        ]), this._bumpSequenceId(1), this.write(N), this.write(n);
  }
  // 0.11+ environment
  startTLS(_) {
    this.config.debug && console.log("Upgrading connection to TLS");
    const A = B_.createSecureContext({
    ...
    }), R = this.config.ssl.rejectUnauthorized, n = this.config.ssl.verifyIdentity, I = OA.isIP(this.config.host) ? void 0 : this.config.host;
    let N = !1;
    ...
    const t = B_.connect(
      {
        ...
        requestCert: R,
        checkServerIdentity: n ? B_.checkServerIdentity : function() {
        },
        secureContext: A,
        isServer: !1,
        socket: this.stream,
      ...
      () => {
        if (N = !0, R && typeof I == "string" && n) {
          const i = t.getPeerCertificate(!0), e = B_.checkServerIdentity(
            I,
            i
          );
          if (e) {
    ...
    t.on("error", (i) => {
      N ? this._handleNetworkError(i) : _(i);
    }), t.on("data", (i) => {
  ...
  protocolError(_, A) {
    if (this._closing)
      return;
    const R = new Error(_);
    R.fatal = !0, R.code = A || "PROTOCOL_ERROR", this.emit("error", R);
  }
  get state() {
  ...
  handlePacket(_) {
    ...
    if (this.config.debug && _) {
      ...
      const A = this._command ? this._command._commandName : "(no command)", R = this._command ? this._command.stateName() : "(no command)";
      console.log(
        `${this._internalId} ${this.connectionId} ==> ${A}#${R}(${[_.sequenceId, _.type(), _.length()].join(",")})`
      );
    }
    if (!this._command) {
      if (_.peekByte() === 255) {
        const R = sE.Error.fromPacket(_);
    ...
    if (_) {
      if (this.sequenceId !== _.sequenceId) {
        ...
        A.expected = this.sequenceId, A.received = _.sequenceId, this.emit("warn", A), console.error(A.message);
      }
    ...
    try {
      ...
      this._handleFatalError(A), this.stream.destroy();
    }
  }
  addCommand(_) {
    if (this.config.debug) {
      const A = _.constructor.name;
      console.log(`Add command: ${A}`), _._commandName = A;
    }
  ...
  format(_, A) {
    ...
    return this._resolveNamedPlaceholders(R), M_.format(
      R.sql,
      R.values,
  ...
  escape(_) {
    return M_.escape(_, !1, this.config.timezone);
  }
  escapeId(_) {
    return M_.escapeId(_, !1);
  }
  raw(_) {
    return M_.raw(_);
  }
  _resolveNamedPlaceholders(_) {
    ...
    if (this.config.namedPlaceholders || _.namedPlaceholders) {
      if (Array.isArray(_.values))
        return;
      CA === null && (CA = ut()()), A = CA(_.sql, _.values), _.sql = A[0], _.values = A[1];
    }
  }
  query(_, A, R) {
    let n;
    _.constructor === NE.Query ? n = _ : n = lA.createQuery(_, A, R, this.config), this._resolveNamedPlaceholders(n);
    const I = this.format(
      n.sql,
      n.values !== void 0 ? n.values : []
    );
    return n.sql = I, this.addCommand(n);
  }
  pause() {
  ...
  prepare(_, A) {
    return typeof _ == "string" && (_ = { sql: _ }), this.addCommand(new NE.Prepare(_, A));
  }
  unprepare(_) {
    let A = {};
    typeof _ == "object" ? A = _ : A.sql = _;
    const R = lA.statementKey(A), n = this._statements.get(R);
    return n && (this._statements.delete(R), n.close()), n;
  }
  execute(_, A, R) {
    let n = {
      infileStreamFactory: this.config.infileStreamFactory
    };
    if (typeof _ == "object" ? (n = {
      ...n,
      ..._,
      sql: _.sql,
      values: _.values
    }, typeof A == "function" ? R = A : n.values = n.values || A) : typeof A == "function" ? (R = A, n.sql = _, n.values = void 0) : (n.sql = _, n.values = A), this._resolveNamedPlaceholders(n), n.values) {
    ...
    const I = new NE.Execute(n, R), N = new NE.Prepare(n, (t, i) => {
      if (t) {
        I.start = function() {
          return null;
        }, R ? R(t) : I.emit("error", t), I.emit("end");
        return;
      }
      I.statement = i;
    });
    return this.addCommand(N), this.addCommand(I), I;
  }
  changeUser(_, A) {
    !A && typeof _ == "function" && (A = _, _ = {});
    const R = _.charset ? Mt.getCharsetNumber(_.charset) : this.config.charsetNumber;
  ...
  ping(_) {
    return this.addCommand(new NE.Ping(_));
  }
  _registerSlave(_, A) {
    return this.addCommand(new NE.RegisterSlave(_, A));
  }
  _binlogDump(_, A) {
    return this.addCommand(new NE.BinlogDump(_, A));
  }
  ...
  close() {
    this.connectTimeout && (mE.clearTimeout(this.connectTimeout), this.connectTimeout = null), this._closing = !0, this.stream.end(), this.addCommand = this._addCommandClosedState;
  }
  createBinlogStream(_) {
    let A = 1;
    const R = new St({ objectMode: !0 });
    return R._read = function() {
      return {
        data: A++
      };
    }, this._registerSlave(_, () => {
      const n = this._binlogDump(_);
      n.on("event", (I) => {
        R.push(I);
      }), n.on("eof", () => {
        R.push(null), _.flags && _.flags & 1 && this.close();
      });
    }), R;
  }
  connect(_) {
  ...
  // ===================================
  writeColumns(_) {
    this.writePacket(sE.ResultSetHeader.toPacket(_.length)), _.forEach((A) => {
      this.writePacket(
        sE.ColumnDefinition.toPacket(A, this.serverConfig.encoding)
      );
    }), this.writeEof();
  }
  ...
  writeTextRow(_) {
    this.writePacket(
      sE.TextRow.toPacket(_, this.serverConfig.encoding)
    );
  }
  writeBinaryRow(_) {
    this.writePacket(
      sE.BinaryRow.toPacket(_, this.serverConfig.encoding)
    );
  }
  writeTextResult(_, A, R = !1) {
    this.writeColumns(A), _.forEach((n) => {
      const I = new Array(A.length);
      A.forEach((N) => {
        I.push(n[N.name]);
      }), R ? this.writeBinaryRow(I) : this.writeTextRow(I);
    }), this.writeEof();
  }
  writeEof(_, A) {
    this.writePacket(sE.EOF.toPacket(_, A));
  }
  writeOk(_) {
    _ || (_ = { affectedRows: 0 }), this.writePacket(sE.OK.toPacket(_, this.serverConfig.encoding));
  }
  writeError(_) {
    const A = this.serverConfig ? this.serverConfig.encoding : "cesu8";
    this.writePacket(sE.Error.toPacket(_, A));
  }
  serverHandshake(_) {
    return this.serverConfig = _, this.serverConfig.encoding = kR[this.serverConfig.characterSet], this.addCommand(new NE.ServerHandshake(_));
  }
  // ===============================================================
  end(_) {
    ...
    const A = this.addCommand(new NE.Quit(_));
    return this.addCommand = this._addCommandClosedState, A;
  }
  static createQuery(_, A, R, n) {
    let I = {
      rowsAsArray: n.rowsAsArray,
      infileStreamFactory: n.infileStreamFactory
    };
    return typeof _ == "object" ? (I = {
      ...I,
      ..._,
      sql: _.sql,
      values: _.values
    }, typeof A == "function" ? R = A : A !== void 0 && (I.values = A)) : typeof A == "function" ? (R = A, I.sql = _, I.values = void 0) : (I.sql = _, I.values = A), new NE.Query(I, R);
  }
...
zA.statementKey = JA.statementKey;
var Xn = zA;
zA.prototype._realEnd = JA.prototype.end;
function Ut(E, _, A) {
  return function(R, n, I) {
    R ? (A.message = R.message, A.code = R.code, A.errno = R.errno, A.sql = R.sql, A.sqlState = R.sqlState, A.sqlMessage = R.sqlMessage, _(A)) : E([n, I]);
  };
}
var N_ = Ut;
const gt = N_;
let lt = class {
  ...
  execute(_) {
    const A = this.statement, R = new Error();
    return new this.Promise((n, I) => {
      const N = gt(n, I, R);
...
function ft(E, _, A) {
  const R = {};
  _.on("newListener", (n) => {
    A.indexOf(n) >= 0 && !_.listenerCount(n) && E.on(
      n,
      R[n] = function() {
        const I = [].slice.call(arguments);
        I.unshift(n), _.emit.apply(_, I);
      }
    );
  }).on("removeListener", (n) => {
    A.indexOf(n) >= 0 && !_.listenerCount(n) && (E.removeListener(n, R[n]), delete R[n]);
  });
}
var ZA = ft;
const rt = GE.EventEmitter, dt = st, $E = N_, Gt = ZA, qR = G_;
let sA = class extends rt {
  constructor(_, A) {
    super(), this.connection = _, this.Promise = A || Promise, Gt(_, this, [
      "error",
      "drain",
      "connect",
      "end",
  ...
  query(_, A) {
    const R = this.connection, n = new Error();
    ...
    return new this.Promise((I, N) => {
      const t = $E(I, N, n);
      A !== void 0 ? R.query(_, A, t) : R.query(_, t);
    });
  }
  execute(_, A) {
    const R = this.connection, n = new Error();
    ...
    return new this.Promise((I, N) => {
      const t = $E(I, N, n);
      A !== void 0 ? R.execute(_, A, t) : R.execute(_, t);
    });
  }
  end() {
    return new this.Promise((_) => {
  ...
  beginTransaction() {
    const _ = this.connection, A = new Error();
    return new this.Promise((R, n) => {
      const I = $E(R, n, A);
      _.beginTransaction(I);
    });
  }
  commit() {
    const _ = this.connection, A = new Error();
  ...
  ping() {
    ...
    return new this.Promise((R, n) => {
      _.ping((I) => {
        I ? (A.message = I.message, A.code = I.code, A.errno = I.errno, A.sqlState = I.sqlState, A.sqlMessage = I.sqlMessage, n(A)) : R(!0);
      });
    });
  }
  connect() {
    ...
    return new this.Promise((R, n) => {
      _.connect((I, N) => {
        I ? (A.message = I.message, A.code = I.code, A.errno = I.errno, A.sqlState = I.sqlState, A.sqlMessage = I.sqlMessage, n(A)) : R(N);
      });
    });
  }
  prepare(_) {
    const A = this.connection, R = this.Promise, n = new Error();
    return new this.Promise((I, N) => {
      A.prepare(_, (t, i) => {
        if (t)
          n.message = t.message, n.code = t.code, n.errno = t.errno, n.sqlState = t.sqlState, n.sqlMessage = t.sqlMessage, N(n);
        else {
          const e = new dt(
            i,
            R
  ...
  changeUser(_) {
    const A = this.connection, R = new Error();
    return new this.Promise((n, I) => {
      A.changeUser(_, (N) => {
        N ? (R.message = N.message, R.code = N.code, R.errno = N.errno, R.sqlState = N.sqlState, R.sqlMessage = N.sqlMessage, I(R)) : n();
      });
    });
  }
  get config() {
...
(function(E) {
  for (let _ = 0; E && _ < E.length; _++) {
    const A = E[_];
    typeof qR.prototype[A] == "function" && sA.prototype[A] === void 0 && (sA.prototype[A] = /* @__PURE__ */ function(n) {
      return function() {
        return qR.prototype[n].apply(
          this.connection,
          arguments
        );
      };
...
var kA = sA;
const wt = kA, Ft = Xn;
let Qt = class extends wt {
  constructor(_, A) {
    super(_, A);
  }
  destroy() {
    return Ft.prototype.destroy.apply(
      this.connection,
      arguments
    );
  }
...
var bt = Vt;
const E_ = d_, DA = GA, ot = GE.EventEmitter, Yt = bt, uA = cn, mt = G_;
function pR(E, _) {
  const A = E.length;
  for (let R = 0; R < A; R++)
    if (E.get(R) === _) {
...
let Wt = class extends ot {
  constructor(_) {
    super(), this.config = _.config, this.config.connectionConfig.pool = this, this._allConnections = new uA(), this._freeConnections = new uA(), this._connectionQueue = new uA(), this._closed = !1, this.config.maxIdle < this.config.connectionLimit && this._removeIdleTimeoutConnections();
  }
  getConnection(_) {
    if (this._closed)
      return E_.nextTick(() => _(new Error("Pool is closed.")));
    let A;
    return this._freeConnections.length > 0 ? (A = this._freeConnections.pop(), this.emit("acquire", A), E_.nextTick(() => _(null, A))) : this.config.connectionLimit === 0 || this._allConnections.length < this.config.connectionLimit ? (A = new Yt(this, {
      config: this.config.connectionConfig
    }), this._allConnections.push(A), A.connect((R) => this._closed ? _(new Error("Pool is closed.")) : R ? _(R) : (this.emit("connection", A), this.emit("acquire", A), _(null, A)))) : this.config.waitForConnections ? this.config.queueLimit && this._connectionQueue.length >= this.config.queueLimit ? _(new Error("Queue limit reached.")) : (this.emit("enqueue"), this._connectionQueue.push(_)) : E_.nextTick(() => _(new Error("No connections available.")));
  }
  releaseConnection(_) {
    let A;
    _._pool ? this._connectionQueue.length ? (A = this._connectionQueue.shift(), E_.nextTick(A.bind(null, null, _))) : (this._freeConnections.push(_), this.emit("release", _)) : this._connectionQueue.length && (A = this._connectionQueue.shift(), E_.nextTick(this.getConnection.bind(this, A)));
  }
  end(_) {
    this._closed = !0, clearTimeout(this._removeIdleTimeoutConnectionsTimer), typeof _ != "function" && (_ = function(N) {
      if (N)
        throw N;
    });
    let A = !1, R = 0, n;
    const I = (function(N) {
      if (!A && (N || ++R >= this._allConnections.length)) {
        A = !0, _(N);
        return;
      }
    }).bind(this);
    ...
    for (let N = 0; N < this._allConnections.length; N++)
      n = this._allConnections.get(N), n._realEnd(I);
  }
  query(_, A, R) {
    const n = mt.createQuery(
      _,
      A,
      R,
      this.config.connectionConfig
    );
    return typeof n.namedPlaceholders > "u" && (n.namedPlaceholders = this.config.connectionConfig.namedPlaceholders), this.getConnection((I, N) => {
      if (I) {
        typeof n.onResult == "function" ? n.onResult(I) : n.emit("error", I);
        return;
      }
      try {
        N.query(n).once("end", () => {
  ...
  execute(_, A, R) {
    typeof A == "function" && (R = A, A = []), this.getConnection((n, I) => {
      if (n)
        return R(n);
      try {
        I.execute(_, A, R).once("end", () => {
          I.release();
        });
      } catch (N) {
        return I.release(), R(N);
      }
    });
  }
  _removeConnection(_) {
    pR(this._allConnections, _), pR(this._freeConnections, _), this.releaseConnection(_);
  }
  ...
  escapeId(_) {
    return DA.escapeId(_, !1);
  }
};
var Kn = Wt;
const Ht = GE.EventEmitter, xR = N_, $R = w_, yt = ZA, En = Kn;
class fA extends Ht {
  constructor(_, A) {
    super(), this.pool = _, this.Promise = A || Promise, yt(_, this, ["acquire", "connection", "enqueue", "release"]);
  }
  getConnection() {
    const _ = this.pool;
    return new this.Promise((A, R) => {
      _.getConnection((n, I) => {
        n ? R(n) : A(new $R(I, this.Promise));
      });
    });
  }
  releaseConnection(_) {
    _ instanceof $R && _.release();
  }
  query(_, A) {
    const R = this.pool, n = new Error();
    if (typeof A == "function")
      throw new Error(
        "Callback function is not available with promise clients."
      );
  ...
  execute(_, A) {
    ...
    return new this.Promise((I, N) => {
      const t = xR(I, N, n);
      A ? R.execute(_, A, t) : R.execute(_, t);
    });
  }
  end() {
    const _ = this.pool, A = new Error();
    return new this.Promise((R, n) => {
      _.end((I) => {
        I ? (A.message = I.message, A.code = I.code, A.errno = I.errno, A.sqlState = I.sqlState, A.sqlMessage = I.sqlMessage, n(A)) : R();
      });
    });
  }
}
(function(E) {
  for (let _ = 0; E && _ < E.length; _++) {
    const A = E[_];
    typeof En.prototype[A] == "function" && fA.prototype[A] === void 0 && (fA.prototype[A] = /* @__PURE__ */ function(n) {
      return function() {
        return En.prototype[n].apply(this.pool, arguments);
      };
    }(A));
  }
})([
...
var Zn = Jt;
const A_ = d_, zt = Jn, Zt = zn, kt = Zn, qt = GE.EventEmitter, kn = {
  RR() {
    let E = 0;
  ...
  let E;
  return typeof A_.hrtime == "function" ? (E = A_.hrtime(), E = E[0] * 1e3 + E[1] * 1e-6) : E = A_.uptime() * 1e3, Math.floor(E);
}, pt = function(E) {
...
class xt {
  constructor(_, A, R) {
    this._cluster = _, this._pattern = A, this._selector = kn[R]();
  }
  getConnection(_) {
    const A = this._getClusterNode();
    if (A === null) {
      let R = new Error("Pool does Not exist.");
      return R.code = "POOL_NOEXIST", this._cluster._findNodeIds(this._pattern, !0).length !== 0 && (R = new Error("Pool does Not have online node."), R.code = "POOL_NONEONLINE"), _(R);
    }
    return this._cluster._getConnection(A, (R, n) => R ? this._cluster._canRetry && this._cluster._findNodeIds(this._pattern).length !== 0 ? (this._cluster.emit("warn", R), this.getConnection(_)) : _(R) : _(null, n));
  }
  /**
   * pool cluster query
   * @param {*} sql
   ...
   */
  query(_, A, R) {
    const n = kt.createQuery(_, A, R, {});
    return this.getConnection((I, N) => {
      if (I) {
        typeof n.onResult == "function" ? n.onResult(I) : n.emit("error", I);
        return;
      }
  ...
  _getClusterNode() {
    ...
    if (_.length === 0)
      return null;
    const A = _.length === 1 ? _[0] : this._selector(_);
    return this._cluster._getNode(A);
  }
}
let $t = class extends qt {
  constructor(_) {
  ...
  of(_, A) {
    _ = _ || "*", A = A || this._defaultSelector, A = A.toUpperCase(), !kn[A] === "undefined" && (A = this._defaultSelector);
    const R = _ + A;
    return typeof this._namespaces[R] > "u" && (this._namespaces[R] = new xt(this, _, A)), this._namespaces[R];
  }
  add(_, A) {
    typeof _ == "object" && (A = _, _ = `CLUSTER::${++this._lastId}`), typeof this._nodes[_] > "u" && (this._nodes[_] = {
      id: _,
      errorCount: 0,
      pool: new zt({ config: new Zt(A) }),
      _offlineUntil: 0
    }, this._serviceableNodeIds.push(_), this._clearFindCaches());
  }
  remove(_) {
    const A = this._findNodeIds(_, !0);
    for (let R = 0; R < A.length; R++) {
      const n = this._getNode(A[R]);
      n && this._removeNode(n);
    }
  }
  getConnection(_, A, R) {
    let n;
    typeof _ == "function" ? (R = _, n = this.of()) : (typeof A == "function" && (R = A, A = this._defaultSelector), n = this.of(_, A)), n.getConnection(R);
  }
  end(_) {
    const A = _ !== void 0 ? _ : (N) => {
      if (N)
        throw N;
    };
    if (this._closed) {
    ...
    for (const N in this._nodes)
      n++, this._nodes[N].pool.end(I);
    n === 0 && A_.nextTick(I);
  }
  _findNodeIds(_, A) {
    let R = 0, n = this._findCaches[_];
    if (n === void 0) {
      const I = pt(_);
      n = this._serviceableNodeIds.filter(
        (N) => N.match(I)
      );
    }
    return this._findCaches[_] = n, A ? n : n.filter((I) => {
      const N = this._getNode(I);
      return N._offlineUntil ? (R || (R = An()), N._offlineUntil <= R) : !0;
    });
  }
  _getNode(_) {
  ...
  _increaseErrorCount(_) {
    const A = ++_.errorCount;
    if (!(this._removeNodeErrorCount > A)) {
      if (this._restoreNodeTimeout > 0) {
        _._offlineUntil = An() + this._restoreNodeTimeout, this.emit("offline", _.id);
        return;
      }
  ...
  _decreaseErrorCount(_) {
    let A = _.errorCount;
    A > this._removeNodeErrorCount && (A = this._removeNodeErrorCount), A < 1 && (A = 1), _.errorCount = A - 1, _._offlineUntil && (_._offlineUntil = 0, this.emit("online", _.id));
  }
  _getConnection(_, A) {
    _.pool.getConnection((R, n) => R ? (this._increaseErrorCount(_), A(R)) : (this._decreaseErrorCount(_), n._clusterId = _.id, A(null, n)));
  }
  _removeNode(_) {
    const A = this._serviceableNodeIds.indexOf(_.id);
    A !== -1 && (this._serviceableNodeIds.splice(A, 1), delete this._nodes[_.id], this._clearFindCaches(), _.pool.end());
  }
  _clearFindCaches() {
    this._findCaches = {};
  }
...
const Ei = Zn, _i = jA;
function Ai(E) {
  return new Ei({ config: new _i(E) });
}
var Ri = Ai;
const ni = Jn, Ii = zn;
function Ni(E) {
  return new ni({ config: new Ii(E) });
}
var Ti = Ni;
const ei = qn;
function ti(E) {
...
class Oi {
  constructor(_, A) {
    this.poolNamespace = _, this.Promise = A || Promise;
  }
  getConnection() {
    const _ = this.poolNamespace;
    return new this.Promise((A, R) => {
      _.getConnection((n, I) => {
        n ? R(n) : A(new ai(I, this.Promise));
      });
    });
  }
  query(_, A) {
    const R = this.poolNamespace, n = new Error();
    if (typeof A == "function")
      throw new Error(
        "Callback function is not available with promise clients."
      );
...
var Ci = Oi;
(function(E) {
  const _ = GA, A = GE.EventEmitter, R = wA, n = qn, I = Ri, N = Ti, t = ii, i = kA, e = jn, T = N_, a = w_, u = ZA, S = Ci;
  function O(M) {
    const L = I(M), B = new Error(), s = M.Promise || Promise;
    if (!s)
      throw new Error(
    ...
    return new s((l, d) => {
      L.once("connect", () => {
        ...
        B.message = o.message, B.code = o.code, B.errno = o.errno, B.sqlState = o.sqlState, d(B);
      });
    });
  }
  function D(M) {
    const L = N(M), B = M.Promise || Promise;
    if (!B)
      throw new Error(
        "no Promise implementation available.Use promise-enabled node version or pass userland Promise implementation as parameter, for example: { Promise: require('bluebird') }"
      );
  ...
  class c extends A {
    constructor(L, B) {
      super(), this.poolCluster = L, this.Promise = B || Promise, u(L, this, ["warn", "remove", "online", "offline"]);
    }
    getConnection(L, B) {
      const s = this.poolCluster;
      return new this.Promise((l, d) => {
        s.getConnection(
          L,
          B,
          (o, Z) => {
            o ? d(o) : l(new a(Z, this.Promise));
          }
        );
      });
    }
    query(L, B) {
      const s = this.poolCluster, l = new Error();
      if (typeof B == "function")
        throw new Error(
          "Callback function is not available with promise clients."
        );
      return new this.Promise((d, o) => {
        const Z = T(d, o, l);
        s.query(L, B, Z);
      });
    }
    ...
    of(L, B) {
      return new S(
        this.poolCluster.of(L, B),
        this.Promise
      );
    }
    end() {
      const L = this.poolCluster, B = new Error();
      return new this.Promise((s, l) => {
        L.end((d) => {
          d ? (B.message = d.message, B.code = d.code, B.errno = d.errno, B.sqlState = d.sqlState, B.sqlMessage = d.sqlMessage, l(B)) : s();
        });
      });
    }
  }
  (function(M) {
    for (let L = 0; M && L < M.length; L++) {
      const B = M[L];
      typeof n.prototype[B] == "function" && c.prototype[B] === void 0 && (c.prototype[B] = /* @__PURE__ */ function(l) {
        return function() {
          return n.prototype[l].apply(
            this.poolCluster,
            arguments
          );
        };
  ...
  function h(M) {
    const L = t(M), B = M && M.Promise || Promise;
    if (!B)
      throw new Error(
        "no Promise implementation available.Use promise-enabled node version or pass userland Promise implementation as parameter, for example: { Promise: require('bluebird') }"
      );
  ...
  E.createConnection = O, E.createPool = D, E.createPoolCluster = h, E.escape = _.escape, E.escapeId = _.escapeId, E.format = _.format, E.raw = _.raw, E.PromisePool = e, E.PromiseConnection = i, E.PromisePoolConnection = a, E.__defineGetter__("Types", () => UE()), E.__defineGetter__(
    "Charsets",
    () => ZE()
  ), E.__defineGetter__(
    "CharsetToEncoding",
    () => SE()
  ), E.setMaxParserCache = function(M) {
    R.setMaxCache(M);
  }, E.clearParserCache = function() {
    R.clearCache();
  };
})(Dn);
const pn = /* @__PURE__ */ CI(Dn), fE = {
  DB_HOST: "localhost",
  DB_PORT: 3306,
  DB_USER: "root",
  DB_PASSWORD: "",
...
function Di() {
  return aE.isPackaged ? x.join(x.dirname(aE.getPath("exe")), "config.json") : x.join(process.cwd(), "config.json");
}
function xn() {
  const E = Di();
  if (!P_.existsSync(E))
    return P_.writeFileSync(E, JSON.stringify(fE, null, 2), "utf-8"), console.log("[config] created default config at:", E), { ...fE };
  try {
    const _ = P_.readFileSync(E, "utf-8"), A = JSON.parse(_);
    return console.log("[config] loaded from:", E), {
      DB_HOST: A.DB_HOST ?? fE.DB_HOST,
      DB_PORT: Number(A.DB_PORT) || fE.DB_PORT,
      DB_USER: A.DB_USER ?? fE.DB_USER,
      DB_PASSWORD: A.DB_PASSWORD ?? fE.DB_PASSWORD,
      DB_NAME: A.DB_NAME ?? fE.DB_NAME
    };
  } catch (_) {
    return console.error("[config] failed to read config, using defaults:", _), { ...fE };
  }
}
const dE = xn();
console.log("[db] connecting to:", dE.DB_HOST, dE.DB_PORT, dE.DB_NAME);
const X = pn.createPool({
  host: dE.DB_HOST,
  port: dE.DB_PORT,
  user: dE.DB_USER,
...
async function ui() {
  const E = xn(), _ = await pn.createConnection({
    host: E.DB_HOST,
    port: E.DB_PORT,
    user: E.DB_USER,
  ...
  await _.execute(`
    CREATE DATABASE IF NOT EXISTS \`${E.DB_NAME}\`
    DEFAULT CHARACTER SET utf8mb4
    DEFAULT COLLATE utf8mb4_unicode_ci
  `), await _.end(), await X.execute(`
  ...
  `), await X.execute("CREATE INDEX idx_reminders_enabled ON reminders(enabled)").catch(() => {
  }), await X.execute("CREATE INDEX idx_reminders_updatedAt ON reminders(updatedAt)").catch(() => {
  }), await X.execute("ALTER TABLE notes ADD COLUMN pinned TINYINT(1) NOT NULL DEFAULT 0").catch(() => {
  }), await X.execute("ALTER TABLE reminders ADD COLUMN pinned TINYINT(1) NOT NULL DEFAULT 0").catch(() => {
  }), await X.execute("ALTER TABLE reminders ADD COLUMN minutes INT NULL").catch(() => {
  }), await X.execute("ALTER TABLE reminders ADD COLUMN `date` DATE NULL").catch(() => {
  }), await X.execute("ALTER TABLE reminders ADD COLUMN `time` TIME NULL").catch(() => {
  }), await X.execute("ALTER TABLE reminders ADD COLUMN days INT NULL").catch(() => {
  }), await X.execute("ALTER TABLE reminders MODIFY COLUMN `date` DATE NULL").catch(() => {
  }), await X.execute("ALTER TABLE reminders MODIFY COLUMN `time` TIME NULL").catch(() => {
  }), await X.execute(`
...
async function Bi(E) {
  return await X.execute("DELETE FROM reminders WHERE id = ?", [E]), !0;
}
async function Mi() {
  const [E] = await X.query(`
    SELECT *
...
function hi(E) {
  ...
  const _ = E.getFullYear(), A = String(E.getMonth() + 1).padStart(2, "0"), R = String(E.getDate()).padStart(2, "0");
  return `${_}-${A}-${R}`;
}
function EI(E) {
...
function Ui(E, _) {
  const A = hi(E);
  if (!A) return null;
  const R = EI(_), n = (/* @__PURE__ */ new Date(`${A}T${R}`)).getTime();
...
function gi(E) {
  ...
  if (E.type === "DATE_TIME")
    return Ui(E.date, E.time);
  const _ = E.days ?? 1, A = E.lastTriggeredAt ?? E.updatedAt ?? E.createAt, R = new Date(A);
  R.setDate(R.getDate() + _);
  const n = `${R.getFullYear()}-${String(R.getMonth() + 1).padStart(2, "0")}-${String(R.getDate()).padStart(2, "0")}`, I = EI(E.time), N = (/* @__PURE__ */ new Date(`${n}T${I}`)).getTime();
  return Number.isNaN(N) ? null : N;
}
function li(E, _) {
  const A = gi(E);
  return !(!A || _ < A || E.lastTriggeredAt && E.lastTriggeredAt >= A);
}
function si(E, _) {
  let A = null, R = !1;
  const n = async () => {
    if (!R) {
      R = !0;
      try {
        const I = Date.now(), N = await Mi();
        let t = !1;
        for (const i of N)
          li(i, I) && (await E({
            title: i.title || "M Note",
            text: i.text || "",
            mode: i.mode
          }), await $n(i.id, I), (i.type === "AFTER_MINUTES" || i.type === "DATE_TIME") && await Li(i.id, I), t = !0);
        t && _ && _();
      } catch (I) {
        console.error("[scheduler.tick] failed:", I);
      } finally {
        R = !1;
      }
    }
...
let z = null;
function fi(E, _, A, R) {
  const { width: n, height: I } = Nn.getPrimaryDisplay().workAreaSize;
  if (z && !z.isDestroyed()) {
    z.show(), z.focus();
    return;
  }
  let N = !1;
  z = new f_({
    width: Math.round(n * 0.45),
    height: Math.round(I * 0.55),
    show: !1,
    alwaysOnTop: !0,
    autoHideMenuBar: !0,
    webPreferences: {
      preload: x.join(A, "preload.mjs")
    }
  }), E ? z.loadURL(`${E}#/quick`) : z.loadFile(x.join(_, "index.html"), { hash: "/quick" }), z.once("ready-to-show", () => {
    z == null || z.show(), z == null || z.focus();
  }), z.on("close", (t) => {
    N || (t.preventDefault(), Promise.resolve(R == null ? void 0 : R()).finally(() => {
      N = !0, z == null || z.close();
    }));
  }), z.on("closed", () => {
    z = null;
...
async function ri(E) {
  const { id: _, title: A, content: R, updatedAt: n, createAt: I, pinned: N } = E;
  return await X.execute(
...
async function Gi(E) {
  return await X.execute("DELETE FROM notes WHERE id = ?", [E]), !0;
}
async function wi() {
  return di();
}
async function qA(E) {
  ...
  return await ri({
    ...
    content: E.content ?? "",
    createAt: E.createAt ?? _,
    updatedAt: E.updatedAt ?? _,
    pinned: E.pinned ?? 0
  }), !0;
}
async function Fi(E) {
...
async function oi(E, _ = Date.now()) {
  return await $n(E, _), !0;
}
async function Yi() {
  const E = Date.now(), A = {
    id: E,
...
async function Hi(E) {
  var I;
  const _ = ((I = s_.title) == null ? void 0 : I.trim()) ?? "", A = s_.content ?? "";
  if (!_ && !A) return;
  const R = Date.now();
  await qA({
    id: R,
    ...
    updatedAt: R,
    pinned: 0
  }), E == null || E.webContents.send("notes:changed"), Wi();
}
console.log("[main] main.ts loaded");
const F_ = x.dirname(NI(import.meta.url));
process.env.APP_ROOT = x.join(F_, "..");
const JE = process.env.VITE_DEV_SERVER_URL, ra = x.join(process.env.APP_ROOT, "dist-electron"), Q_ = x.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = JE ? x.join(process.env.APP_ROOT, "public") : Q_;
let F = null, R_ = !1, L_ = null, rA = "Alt+Space";
const AI = si(
  async (E) => {
    E.mode === "NOTIFICATION" ? new Tn({
      title: E.title,
      body: E.text
    }).show() : E.mode === "POPUP_WINDOW" && RI(E.text);
  },
  () => {
    F == null || F.webContents.send("reminders:changed");
  }
);
process.platform === "win32" && aE.setAppUserModelId("com.yourapp.mnote");
function nn() {
  F = new f_({
    icon: x.join(process.env.VITE_PUBLIC, "electron-vite.svg"),
    webPreferences: {
      preload: x.join(F_, "preload.mjs")
    }
  }), F.on("close", (E) => {
    R_ || (E.preventDefault(), F == null || F.hide());
  }), F.webContents.on("did-finish-load", () => {
    F == null || F.webContents.send("main-process-message", (/* @__PURE__ */ new Date()).toLocaleString());
  }), JE ? F.loadURL(`${JE}#/`) : F.loadFile(x.join(Q_, "index.html"), { hash: "/" });
}
function yi(E) {
  return aE.isPackaged ? x.join(process.resourcesPath, E) : x.join(process.cwd(), "resources", E);
}
function vi() {
  F == null || F.webContents.send("app:save-before-close"), setTimeout(() => {
    R_ || (R_ = !0, aE.quit());
  }, 1e4);
}
function Xi() {
  const E = yi("tray.ico"), _ = nI.createFromPath(E);
  L_ = new II(_), L_.setToolTip("MNote");
  const A = en.buildFromTemplate([
    {
      label: "Main Window",
      click: () => {
        F && (F.show(), F.focus());
  ...
  L_.setContextMenu(A), L_.on("click", () => {
    F && (F.isVisible() ? F.hide() : (F.show(), F.focus()));
  });
}
function In(E) {
  return SA.unregisterAll(), SA.register(E, () => {
    console.log("[main] hotkey triggered:", E), fi(JE, Q_, F_, () => Hi(F));
  }) ? (rA = E, !0) : !1;
}
function RI(E) {
  const _ = `reminder:submit-mandatory:${TI()}`, { width: A, height: R } = Nn.getPrimaryDisplay().workAreaSize, n = !!F && !F.isDestroyed(), I = new f_({
    width: Math.round(A * 0.6),
    height: Math.round(R * 0.65),
    ...n ? { parent: F, modal: !0 } : {},
    center: !0,
    resizable: !1,
    minimizable: !1,
  ...
  I.show(), I.focus(), JE ? I.loadURL(
    `${JE}#/reminder-mandatory?text=${encodeURIComponent(E)}&channel=${_}`
  ) : I.loadFile(x.join(Q_, "index.html"), {
    hash: `/reminder-mandatory?text=${encodeURIComponent(E)}&channel=${_}`
  });
  let N = !1;
  I.on("close", (t) => {
    N || t.preventDefault();
  }), p.handleOnce(_, async (t, i) => (N = !0, I.close(), p.removeHandler(_), !0));
}
async function Ki() {
  try {
    await aE.whenReady(), console.log("[env] DB_HOST:", process.env.DB_HOST), console.log("[env] DB_USER:", process.env.DB_USER), console.log("[env] DB_PASSWORD:", process.env.DB_PASSWORD ? "***有值***" : "***空***"), console.log("[env] DB_NAME:", process.env.DB_NAME), await ui(), console.log("[main] schema init ok"), p.on("app:save-done", () => {
      R_ = !0, F == null || F.close();
    }), p.handle("shortcut:update", (E, _) => In(_)), p.handle("shortcut:get", () => rA), p.on("quick:note:update", (E, _) => {
      mi(_);
    }), p.handle("reminder:show", (E, _) => (new Tn({
      title: _.title || "Reminder",
      body: _.body || ""
    }).show(), !0)), p.handle("reminder:open-mandatory", (E, _) => (RI(_ || ""), !0)), p.handle("notes:getAll", async () => wi()), p.handle("notes:upsert", async (E, _) => qA(_)), p.handle("notes:delete", async (E, _) => Fi(_)), p.handle("notes:create", async () => Qi()), p.handle("reminders:getAll", async () => Vi()), p.handle("reminders:upsert", async (E, _) => _I(_)), p.handle("reminders:delete", async (E, _) => bi(_)), p.handle("reminder:create", async () => Yi()), p.handle(
      "reminders:markTriggered",
      async (E, _, A) => oi(_, A)
    ), en.setApplicationMenu(null), nn(), In(rA), Xi(), AI.start(), aE.on("activate", () => {
      f_.getAllWindows().length === 0 && nn();
    });
  } catch (E) {
    console.error("[main] bootstrap failed:", E), aE.quit();
  }
}
Ki();
aE.on("window-all-closed", () => {
  process.platform !== "darwin" && (aE.quit(), F = null);
});
aE.on("before-quit", () => {
  R_ = !0;
});
aE.on("will-quit", () => {
  AI.stop(), SA.unregisterAll();
});
export {
  ra as MAIN_DIST,
  Q_ as RENDERER_DIST,
  JE as VITE_DEV_SERVER_URL
};
